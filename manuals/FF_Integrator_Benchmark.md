...
The integrator peak fitting benchmark (`tests/test_calibration_integration.py`) validates the complete **calibration → integration → peak fitting** pipeline by:
...
```bash
python tests/test_calibration_integration.py [-nCPUs N] [--skip-calibration] [--max-rings N]
```
...
python tests/test_calibration_integration.py -nCPUs 8

# Quick iteration (skip calibration, use existing geometry)
python tests/test_calibration_integration.py -nCPUs 8 --skip-calibration

# Fit more rings for more thorough validation
python tests/test_calibration_integration.py -nCPUs 8 --max-rings 25
```
