...
| **`argv[4]` (`nWork`) is ignored in PF mode** — `nVoxels = numScans²` from `positions.csv` (`IndexerUnified.c:2932` prints "argv ignored for PF") | a voxel-limited test run silently processes the whole layer; use `blockNr`/`nBlocks` to slice | phase-7 §7.8 |
