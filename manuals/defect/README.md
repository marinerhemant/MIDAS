...
| `phase-4-rods.md` | rods in the cloud and on the frame stack; **which (h,k) rods carry intensity** (the in-plane fault-vector test); an open split / one-sided record; satellites; polytype; **the integrated gap fraction and a coherent-polytype detection limit, each with its own real trap**; **the Friedel/crossing quartet and how to verify a pairing**; variants vs a fixed q-offset; **a ladder can be right in \|q\| and not be a ladder** | fault / polytype work |
