...
| `indexing` (c-omp) | reads those corrected spots. It parses a `DetTx` out of `DetParams` and then never reads it back (`midas_index/c_src/IndexerUnified.c:2857`) — a dead store, not a dropped correction |
