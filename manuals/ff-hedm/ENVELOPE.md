...
**Last checked:** 2026-08-31 · **Owner:** Hemant Sharma (hsharma@anl.gov)
...
| **`UpperBoundThreshold` vs the detector's real ceiling** | **GE5 clips hard at 16 349 counts**, measured 2026-08-31 | one `bt_1id_mar23` CeO2 file (`park_CeO2_3s_..._000099.edf.ge5`, 5 frames of 2048²): **1071–1122 pixels per frame on exactly 16 349**, mean **1090**, and only **11–19 pixels per frame** anywhere in (max−100, max) — a clipping ceiling, not a bright tail ([`RUNBOOK.md`](RUNBOOK.md) §R2f) | The row above only bites if the threshold is **below** the ceiling. `FF_HEDM/Example/Parameters.txt` ships `UpperBoundThreshold 70000`, more than 4× the GE5 ceiling, so on this detector **no saturated peak is ever flagged or dropped** — clipped peaks are fitted as if real, and the loss the row above describes is replaced by a silent bias in the fitted intensity. A run inherits this from the template without a word in the log. | Measure the ceiling per detector — `max` of a bright frame, plus the count of pixels on it — and set `UpperBoundThreshold` from that. **Subtract the dark's own stuck pixels first**: the same scan's dark was reported at **387 pixels at the ceiling**, which against the measured 1090 per data frame would be ~35 % dead detector rather than signal. That ratio is **not yet confirmed** — the dark has not been counted per-frame the same way, and until it is, count the ceiling pixels in your own dark rather than carrying this number. |


> **When the beam is thinner than the grains, "the true per-layer beam" is the wrong
> bound — and this row contradicted `phase-2-configure.md` §6 until 2026-09-03.**
> The bound applies to the grain **centroid**, not to the illuminated material. A grain
> larger than the slab is lit across only part of itself, so its centroid legitimately
> lies up to ±R_grain outside the slab. Setting `Hbeam` to the beam height then forces
> every such centroid onto the boundary — the same artefactual pile-up the hard rule in
> `phase-2-configure.md` warns about, arrived at from the opposite direction.
>
> Measured case: `shade_LSHR` (Sparks et al. 2024) runs a **1.5 µm** beam on LSHR whose
> grains are tens of µm. `Hbeam 1.5` would compress ~30 µm grains into a 1.5 µm plane.
> The generous 2000 µm default is correct there. **Use the true beam only when the beam
> is thicker than the grains**; otherwise keep the generous bound and take the Z
> constraint from the physics, not the parameter.
...
| `Hbeam` / `BeamThickness` | per run | **max(true per-layer beam, a few × the grain size)** — see the caveat below | physics: grains outside the beam cannot diffract | Constrains Z to the illuminated slab. **Never set to the sample dimension** — a 10-layer 100 µm scan carrying `Hbeam 1000` lets Z roam ±500 µm. |
...
| Grain **Z** to better than ~2–3× the beam height | The vertical coordinate is the badly-conditioned one for a thin beam: a grain's spots move little in Z as it moves in Z. Measured on 20-ID with a 100 µm beam (layer step 0.075 mm, so BH100/OL25 confirmed from the data, not the folder name): grain-Z scatter 153 µm where a uniform slab gives 29, improving to 76 after refining `tx`/`Wedge` (§5h). Only 28 % of grains sat within ±50 µm of the beam plane, rising to 44 %. | This is not a bounding-box artefact — the distribution is peaked at the beam and nowhere near the ±500 µm `Hbeam` bound, so rule 9 is not in play. X and Y are unaffected (271/265 µm before, 273/272 after), which is exactly how you tell a resolution limit from a fit absorbing error. **Use X and Y; treat Z as indicative.** **The ratio, not the improvement, is the limit** — see the row below. |

> **Beam thickness decides the SIGN of this.** Measured 2026-09-03 on `shade_LSHR`
> (1-ID GE, **1.5 µm** beam, layer coincident with an EBSD section so true Z is known to a
> 0.43 µm sd): Z error **3.11 µm** against an in-plane error of **5.04 µm** — Z is
> **BETTER** determined than in-plane, 0.62×. The row above, and the 20-ID numbers in it,
> were measured with a **100 µm** beam, where Z genuinely is the loose coordinate. With a
> beam thinner than the grains the beam itself constrains Z — a grain outside the slab
> cannot diffract at all — and the ordering inverts. Do not carry "Z is the
> badly-conditioned coordinate" across beam geometries.

| **"Refining `tx` halves the grain-Z scatter"** — it does not, in general | A `tx`/`Wedge` correction tightens whatever the geometry error was loosening. Where Z is the loose coordinate it improves Z; where Z is already at its conditioning limit there is nothing to recover, and the correction shows up in `DiffPos` instead. Measured on 1-ID `bt_1id_mar23` LSHR (2321 grains, 200 µm beam), a **larger** roll than the 20-ID case above: grain-Z scatter 36.5 → **32.4 µm** — barely moved — while `DiffPos` p50 went 251.3 → **62.3 µm** (4.0×) and its hard floor at 235 µm vanished (p5 238.5 → 20.4). X/Y unchanged (283/285 → 290/291), `DiffOme` unchanged (0.072 → 0.074°). | So the 20-ID "Z halved" number is a **property of that dataset's starting geometry, not an acceptance test for `tx`**. Use `DiffPos` and its low percentiles as the before/after signature (DIAGNOSIS *A hard floor in the `DiffPos` distribution*), and use **`DiffOme` as the control** — a detector roll acts in the detector plane and must not change which ω frame a spot lands on. This row exists because §5h's Z-halving sentence was being read as general; it is not (§5h, Lab Notebook §10c). |
