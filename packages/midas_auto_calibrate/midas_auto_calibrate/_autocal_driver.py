...
# Vendored-into-package note: the original AutoCalibrateZarr.py imports
# diplib BEFORE numpy so that diplib's libomp loads first; reversing that
# order segfaults on macOS. When loaded as part of an already-running
# Python process (e.g. from ``import midas_auto_calibrate``), numpy is
# already in memory — so the diplib path is unsafe. Force the scipy
# fallback; it's ~10× slower on the median filter but numerically
# equivalent (same kernel, same iteration count). Users who run the
# CLI in a fresh process without numpy pre-imported (the plain `python
# AutoCalibrateZarr.py` path) still get the fast path there.
import os as _os
_FORCE_SCIPY_MEDIAN = _os.environ.get(
    "MIDAS_AUTOCAL_USE_DIPLIB", "0") not in ("1", "true", "TRUE")
if _FORCE_SCIPY_MEDIAN:
    _HAS_DIPLIB = False
else:
    try:
...
import argparse
import sys
# Vendored into midas_auto_calibrate: midas_config is a MIDAS-tree helper that
# only does startup checks (disk-space, env). We no-op it — binary discovery
# in this vendored copy goes through midas_auto_calibrate._binaries.midas_bin.
try:
    import midas_config  # type: ignore[import-not-found]
...
# Binary discovery shim for the vendored package copy.
# In the original AutoCalibrateZarr.py, binaries are located at
# `{INSTALL_PATH}/FF_HEDM/bin/{name}`. In the packaged copy they ship
# inside the wheel; `_midas_bin_path()` remaps legacy names (DetectorMapper,
# IntegratorZarrOMP, CalibrantIntegratorOMP) to the wheel-native names
# (MIDASDetectorMapper, MIDASIntegrator, MIDASCalibrant) via
# midas_auto_calibrate._binaries.midas_bin.
from ._binaries import midas_bin as _wheel_midas_bin  # noqa: E402

_BIN_NAME_MAP = {
    "CalibrantIntegratorOMP": "MIDASCalibrant",
    "DetectorMapper": "MIDASDetectorMapper",
    "IntegratorZarrOMP": "MIDASIntegrator",
    "GetHKLList": "GetHKLList",
}


def _midas_bin_path(legacy_name: str) -> str:
    """Return the absolute path of a MIDAS binary, accepting legacy names."""
    wheel_name = _BIN_NAME_MAP.get(legacy_name, legacy_name)
    return str(_wheel_midas_bin(wheel_name))


class _InstallPathShim:
    """Back-compat for ``os.path.join(INSTALL_PATH, 'FF_HEDM/bin/<name>')``.

    The vendored code joins `INSTALL_PATH` with relative paths to the MIDAS
    tree. This shim intercepts the join and returns wheel-resident binaries
    for ``FF_HEDM/bin/*`` lookups, or raises a clear error for
    ``utils/ffGenerateZipRefactor.py`` (the Zarr-conversion helper, which
    isn't shipped in the wheel — users on the ``--convert 0`` path must
    install the MIDAS tree).
    """

    def __fspath__(self) -> str:  # accepts os.PathLike
        return ""
...
def _resolve_install_path(sub: str) -> str:
    """Given a path relative to INSTALL_PATH, return the wheel-resident copy."""
    # FF_HEDM/bin/<name>
    if sub.startswith("FF_HEDM/bin/"):
        return _midas_bin_path(sub.rsplit("/", 1)[-1])
    # utils/ffGenerateZipRefactor.py — Zarr converter, not vendored
    if sub.endswith("ffGenerateZipRefactor.py"):
        raise RuntimeError(
            "The --convert 0 Zarr-conversion path requires the MIDAS tree's "
            "utils/ffGenerateZipRefactor.py, which isn't vendored in the wheel. "
            "Use --convert -1 (auto) or a pre-generated .zip input, or install "
            "MIDAS from source and set MIDAS_INSTALL_DIR."
        )
    # gui/viewers/plot_calibrant_results.py — optional GUI
    if sub.endswith("plot_calibrant_results.py"):
        from importlib import resources
        try:
            return str(resources.files("midas_auto_calibrate.viz") / "viewer.py")
        except (ModuleNotFoundError, FileNotFoundError):
            raise RuntimeError(
                "CalibrantViewer requires the midas_auto_calibrate[viz-gui] "
                "extra (PyQt5)."
            )
    raise RuntimeError(
        f"Vendored AutoCalibrateZarr tried to resolve INSTALL_PATH/{sub} "
        f"which isn't remapped into the wheel."
    )


INSTALL_PATH = _InstallPathShim()
env = dict(os.environ)
...
def run_get_hkl_list_cli(sg, latc, wavelength, lsd, max_ring_rad):
    ...
    """
    hkl_bin = _resolve_install_path('FF_HEDM/bin/GetHKLList')
    cmd = [
        hkl_bin,
        '--sg', str(int(sg)),
...
def generateZip(resFol, pfn, dfn='', darkfn='', dloc='', darkloc='',
    ...
    cmd = [
        pytpath,
        _resolve_install_path('utils/ffGenerateZipRefactor.py'),
        '-resultFolder', resFol,
        '-paramFN', pfn
    ]
    if dfn:
...
def run_get_hkl_list(param_file):
    ...
    try:
        cmd = f"{_resolve_install_path('FF_HEDM/bin/GetHKLList')} {param_file}"
...
def run_integrator_validation(refined_params_file, data_file, dark_file,
    ...
    mapper_bin = _resolve_install_path('FF_HEDM/bin/DetectorMapper')
    integrator_bin = _resolve_install_path('FF_HEDM/bin/IntegratorZarrOMP')
    hkl_bin = _resolve_install_path('FF_HEDM/bin/GetHKLList')
...
def runMIDAS(rawFN, state, n_iterations=40, mult_factor=5,
    ...
    try:
        ...
        calibrant_exe = _resolve_install_path('FF_HEDM/bin/CalibrantIntegratorOMP')
...
def main():
    ...
    try:
        ...
        if os.path.exists(corr_file):
            ...
            if DrawPlots:
                # Launch interactive calibrant viewer (optional [viz-gui] extra).
                try:
                    viewer_script = _resolve_install_path(
                        'gui/viewers/plot_calibrant_results.py')
                    subprocess.Popen(
                        [sys.executable, viewer_script, corr_file],
                        env=env)
                    logger.info(f"Launched CalibrantViewer on {corr_file}")
        ...
        # Vendored addition: dump strain metrics to the refined params file
        # so the programmatic :func:`midas_auto_calibrate.calibrate_zarr`
        # wrapper can read them back without scraping the log.
        if state.mean_strain is not None:
            final_params['MeanStrain'] = state.mean_strain
        if state.std_strain is not None:
            final_params['StdStrain'] = state.std_strain
