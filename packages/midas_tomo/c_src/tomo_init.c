...
void usage() {
  printf(
      ...
      "  --fftw-bridge  GPU mode: use CPU FFTW for the FFTs instead of cuFFT.\n"
      "                 A diagnostic, NOT a bit-parity switch. Measured on an\n"
      "                 RTX A6000 (128x180): --gpu --fftw-bridge differs from the\n"
      "                 CPU FFTW path by 1.0e-5 relative, and plain --gpu differs\n"
      "                 by 1.0e-5 too -- i.e. routing the FFTs through FFTW does\n"
      "                 not close the gap. The CPU FFTW/pocketfft difference is\n"
      "                 only 2.8e-7, so the GPU/CPU residual is in the gridding,\n"
      "                 not the transform. Slower; use it to localise a\n"
      "                 discrepancy, not to reproduce CPU output bit-for-bit.\n"
