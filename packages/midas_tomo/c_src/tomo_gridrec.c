...
float shlo(float x) { return fabs(sin(PI * x) / PI); }
float hann(float x) { return fabs(x) * 0.5 * (1.0 + cos(2 * PI * x)); }
float hamm(float x) { return fabs(x) * (0.54 + 0.46 * cos(2 * PI * x)); }
float ramp(float x) { return fabs(x); }
...
void destroyFFTMemoryStructures(gridrecParams *param) {
  if (param->in_1d != NULL) {
    fftwf_free(param->in_1d);
    param->in_1d = NULL;
    param->out_1d = NULL;
  }
  if (param->backward_plan_1d != NULL) {
...
void fourn(float data[], unsigned long nn[], int ndim, int isign,
  ...
  if ((nx != param->nx_prev) || (ny != param->ny_prev)) {
    ...
    //~ printf("fft_test2f: creating plans, nx=%d, ny=%d, nx_prev=%d,
    ...
    if (param->fft_engine == MIDAS_FFT_POCKET) {
      ...
      if (rc == 1) {
        param->forward_plan_2d =
            fftwf_plan_dft_2d(ny, nx, param->in_2d, param->out_2d, FFTW_FORWARD,
                              FFTW_WISDOM_ONLY);
      } else {
        printf("Creating wisdom file. %s\n", plan2DFN);
        param->forward_plan_2d = fftwf_plan_dft_2d(
            ny, nx, param->in_2d, param->out_2d, FFTW_FORWARD, FFTW_MEASURE);
...
void setSinoAndReconBuffers(int number, float *sinogram_address,
  ...
  if (param->G1 == NULL)
    param->G1 =
        (float **)malloc((size_t)(param->theta_list_size * sizeof(float *)));
  if (param->G2 == NULL)
    param->G2 =
        (float **)malloc((size_t)(param->theta_list_size * sizeof(float *)));
  if (param->S1 == NULL)
    param->S1 = (float **)malloc((size_t)(param->imgsiz * sizeof(float *)));
  if (param->S2 == NULL)
    param->S2 = (float **)malloc((size_t)(param->imgsiz * sizeof(float *)));
  if (number == 1) {
...
void four1(float data[], unsigned long nn, int isign, gridrecParams *param) {
  ...
  if (n != param->n_prev) {
    ...
    if (param->fft_engine == MIDAS_FFT_POCKET) {
      ...
      if (rc == 1) {
        param->backward_plan_1d = fftwf_plan_dft_1d(
            n, param->in_1d, param->out_1d, FFTW_BACKWARD, FFTW_WISDOM_ONLY);
      } else {
        printf("Creating wisdom file. %s\n", plan1DFN);
        param->backward_plan_1d = fftwf_plan_dft_1d(
            n, param->in_1d, param->out_1d, FFTW_BACKWARD, FFTW_MEASURE);
...
void phase1(gridrecParams *param) {
  ...
  float U, V, rtmp, L2 = param->L / 2.0, convolv,
                    tblspcg = 2 * param->ltbl / param->L;
  long pdim2 = param->pdim >> 1, M2 = param->M >> 1, iul, iuh, iu, ivl, ivh, iv,
       n;
  float offset = 0.0;
  complex phfac;
  for (n = 0; n < param->theta_list_size; n++) {
    int j, k;
    if (param->flag)
      offset = (param->X0 * param->COSE[n] + param->Y0 * param->SINE[n]) * PI;
    j = 1;
    ...
    for (j = 1; j < pdim2; j++) {
      ...
      Cmult(Cdata1, Ctmp, param->cproj[j + 1]) Ctmp.i = -Ctmp.i;
      Cmult(Cdata2, Ctmp, param->cproj[(param->pdim - j) + 1]) U =
          (rtmp = param->scale * j) * param->COSE[n] + M2;
      ...
      for (iv = ivl, k = 0; iv <= ivh; iv++, k++)
        param->work[k] = Cnvlvnt(abs(V - iv) * tblspcg, param);
      for (iu = iul; iu <= iuh; iu++) {
        ...
        for (iv = ivl, k = 0; iv <= ivh; iv++, k++) {
          ...
          param->H[(param->M - iu) * param->M + (param->M - iv) + 1].r +=
              convolv * Cdata2.r;
          param->H[(param->M - iu) * param->M + (param->M - iv) + 1].i +=
              convolv * Cdata2.i;
        }
      }
    }
  ...
  while (j < param->M0) {
    for (iu = ustart; iu < ufin; j++, iu++) {
      ...
      while (k < param->M0) {
        ...
        if (k < param->M0)
          (vstart = 0, vfin = param->M02 + 1);
      }
    }
    if (j < param->M0)
      (ustart = 0, ufin = param->M02 + 1);
  }
}
...
void initGridRec(gridrecParams *param) {
  ...
  param->filphase =
      (complex *)malloc(((param->pdim / 2) + 1) * sizeof(complex));
  ...
  param->H =
      (complex *)malloc((param->M + 1) * (param->M + 1) * sizeof(complex));
  ...
  pswf_su(pswf, param->ltbl, param->M02, param->wtbl, param->dwtbl, param->winv,
          param);
