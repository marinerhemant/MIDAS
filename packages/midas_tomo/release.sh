#!/usr/bin/env bash
# Release a new version of midas-tomo.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-tomo-v${NEW_VERSION}"

echo "=== Releasing midas-tomo v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_tomo/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_tomo/__init__.py
rm -f pyproject.toml.bak midas_tomo/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_tomo/__init__.py | cut -d'"' -f2)
...
# KMP_DUPLICATE_LIB_OK: the shared library links its own OpenMP runtime, and
# loading it beside numpy's aborts on macOS without this. Harmless elsewhere.
#
# The engine tests skip unless MIDAS_TOMO is built, so a green run here does
# NOT mean the C is exercised -- on a laptop without FFTW only the pure-Python
# tests ran. Release from a machine with the engine built, or check the skip
# count. The counts to expect:
#   engine + reference binary present : 112 passed
#   no engine at all                  : 89 passed, 3 skipped
KMP_DUPLICATE_LIB_OK=TRUE python -m pytest tests/ -q --tb=short -rs || {
    echo "ERROR: tests failed. Aborting."
    git checkout -- pyproject.toml midas_tomo/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_tomo/__init__.py"
    exit 0
fi
...
VERSION_FILES=(pyproject.toml midas_tomo/__init__.py)

git add -- "${VERSION_FILES[@]}"
if git diff --cached --quiet -- "${VERSION_FILES[@]}"; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-tomo: bump version to ${NEW_VERSION}" \
        -- "${VERSION_FILES[@]}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-tomo v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-tomo v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  PyPI: https://pypi.org/project/midas-tomo/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-tomo && \\"
    echo "          python -c 'import midas_tomo; print(midas_tomo.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-tomo v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo
