#!/usr/bin/env bash
# Release a new version of midas-defect.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-defect-v${NEW_VERSION}"

echo "=== Releasing midas-defect v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_defect/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_defect/__init__.py
rm -f pyproject.toml.bak midas_defect/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_defect/__init__.py | cut -d'"' -f2)
...
KMP_DUPLICATE_LIB_OK=TRUE python -m pytest tests/ -q --tb=short || {
    ...
    git checkout -- pyproject.toml midas_defect/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_defect/__init__.py"
    exit 0
fi
...
VERSION_FILES=(pyproject.toml midas_defect/__init__.py)

git add -- "${VERSION_FILES[@]}"
if git diff --cached --quiet -- "${VERSION_FILES[@]}"; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-defect: bump version to ${NEW_VERSION}" \
        -- "${VERSION_FILES[@]}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-defect v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-defect v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  PyPI: https://pypi.org/project/midas-defect/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-defect && \\"
    echo "          python -c 'import midas_defect; print(midas_defect.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-defect v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo
