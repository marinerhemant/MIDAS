#!/usr/bin/env bash
# Release a new version of midas-joint-ff-calibrate.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-joint-ff-calibrate-v${NEW_VERSION}"

echo "=== Releasing midas-joint-ff-calibrate v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_joint_ff_calibrate/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_joint_ff_calibrate/__init__.py
rm -f pyproject.toml.bak midas_joint_ff_calibrate/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_joint_ff_calibrate/__init__.py | cut -d'"' -f2)
...
python -m pytest tests/ -q --tb=short || {
    ...
    git checkout -- pyproject.toml midas_joint_ff_calibrate/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_joint_ff_calibrate/__init__.py"
    exit 0
fi
...
git tag -a "$TAG" -m "midas-joint-ff-calibrate v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    gh release create "$TAG" dist/* \
        --title "midas-joint-ff-calibrate v${NEW_VERSION}" \
        --generate-notes
    ...
    echo "  PyPI: https://pypi.org/project/midas-joint-ff-calibrate/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-joint-ff-calibrate && \\"
    echo "          python -c 'import midas_joint_ff_calibrate; print(midas_joint_ff_calibrate.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-joint-ff-calibrate v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo
