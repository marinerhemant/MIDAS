...
// customSteps != NULL overrides the dim-based default simplex steps (needed
// for the FF position phase: position is in µm, not Euler degrees, so the
// 0.05 default is meaningless there).
static void RunFitSteps(int dim, double *x0, double *lb, double *ub,
                        double (*objfn)(unsigned, const double *, double *,
                                        void *),
                        void *data, double *xOut, const double *customSteps) {
  int i;
  ...
  if (customSteps) {
    for (i = 0; i < dim; i++) steps[i] = customSteps[i];
  } else if (dim == 9) {
  ...
  run_nlopt_optimization(MIDAS_LN_NELDERMEAD, &config);
  run_nlopt_optimization(MIDAS_LN_NELDERMEAD, &config);
  for (i = 0; i < dim; i++)
    xOut[i] = x[i];
}

// Backward-compatible wrapper: dim-based default steps.
static void RunFit(int dim, double *x0, double *lb, double *ub,
  ...
  RunFitSteps(dim, x0, lb, ub, objfn, data, xOut, NULL);
}

// ════════════════════════════════════════════════════════════════════════
// FF position fit (D1) — SPATIAL objective, ported from FitPosOrStrainsOMP.
// Refines grain position by minimizing CalcNorm2(PosObs - PosTheor) on the
// displacement-corrected detector y,z (NOT the angular metric the orient/strain
// stages use). Orient + lattice are fixed; TheorSpots(y,z,spotid) are
// precomputed once. Matched obs spots arrive in `spotsYZO` (= the PF base's
// SpList: [4]=ome,[5]=y,[6]=z, [8]=matched theor spotid, [9]=mask, [10]=fRMSE).
// ════════════════════════════════════════════════════════════════════════
struct OptimizeScratch {
  double **hkls;           // [nhkls][7]
  double **TheorSpots;     // [MaxNSpotsBest][9]
  double **SpotsYZOGCorr;  // [nSpotsComp][5]
  double **TheorSpotsYZWE; // [MaxNSpotsBest][5]
...
FitErrorsPosSec(double x[3], int nSpotsComp, double spotsYZOIn[nSpotsComp][11],
  ...
  for (i = 0; i < nSpotsComp; i++) {
    ...
    scratch->SpotsYZOGCorr[i][2] = spotsYZO[i][8]; // matched theor spotid
  }
  double **TheorSpotsYZWE = scratch->TheorSpotsYZWE;
  double **SpotsYZOGCorr = scratch->SpotsYZOGCorr;
  double PosObs[2], PosTheor[2], Spnr, Error = 0;
  for (i = 0; i < nSpotsComp; i++) {
...
static double problem_function_Pos(unsigned n, const double *x, double *grad,
  ...
  for (unsigned i = 0; i < n; i++)
    XIn[i] = x[i];
  return FitErrorsPosSec(XIn, nSpotsComp, spotsYZO, f_data->nTspots, f_data->Lsd,
                         f_data->Wavelength, f_data->wedge, f_data->chi,
                         f_data->scratch);
}

// Refine position (3 DOF) with orient + lattice fixed. Writes XFit[3].
static void FitPosSec(double X0[3], int nSpotsComp, double **spotsYZO, int nhkls,
                      double **hkls, double Lsd, double Wavelength,
                      int nOmeRanges, double OmegaRanges[MAXNOMEGARANGES][2],
                      double BoxSizes[MAXNOMEGARANGES][4], double MinEta,
                      double wedge, double chi, double *XFit, double lb[3],
                      double ub[3], double Orient[3], double Strains[6]) {
  unsigned n = 3;
  double x[3], xl[3], xu[3];
  int i, j;
  ...
  // Pre-compute invariant theoretical spots (orient + lattice fixed).
  ...
  free(f_data.scratch->SpotLookup);
  free(f_data.scratch);
}
...
static inline int StrainTensorKenesei(int nspots, double **SpotsInfo,
                                      double Distance,
...
int main(int argc, char *argv[]) {
  ...
  // Seed source (D6): consolidated IndexBest_all.bin (PF / c-omp indexer) vs
  // legacy per-seed IndexBest.bin + IndexBestFull.bin (FF). Detect by file
  // presence — prefer consolidated, fall back to legacy.
  int ffLegacySeeds = 0;
  {
    int tf = open("Output/IndexBest_all.bin", O_RDONLY);
    if (tf >= 0) {
      close(tf);
    } else {
      char legFN[2048];
      sprintf(legFN, "%s/IndexBest.bin", OutputFolder);
      int tf2 = open(legFN, O_RDONLY);
      if (tf2 >= 0) {
        close(tf2);
        ffLegacySeeds = 1;
        printf("Seed source: legacy IndexBest.bin / IndexBestFull.bin (FF).\n");
      }
    }
  }
  ...
  int linesSkipped = (startRowNr >= 1) ? (startRowNr - 1) : 0;
  ...
  /* Load consolidated indexer output files (PF / c-omp index). FF legacy seeds
   * are read per-seed inside the fit loop from IndexBest.bin/IndexBestFull.bin. */
  ...
  double MargOme2 = 2.0 * deg2rad, chi = 0;
  int thisRowNr;

  SpotDiagVoxel *diagVoxels = calloc(nSptIDs, sizeof(SpotDiagVoxel));

  /* FF (D9): per-grain output files ProcessGrains consumes — OrientPosFit.bin
  ...
  if (ffLegacySeeds) {
    ...
    sprintf(ffn, "%s/Key.bin", ResultFolder);
...
#pragma omp parallel for num_threads(numProcs) private(thisRowNr)              \
  ...
  for (thisRowNr = 0; thisRowNr < nSptIDs; thisRowNr++) {
    ...
    int nSpotsBest = 0;
    int *spotIDS = NULL;
    int voxNr = (ffLegacySeeds ? (int)infoArr[thisRowNr * 5 + 0] : -1);
    size_t solIndex = 0;
    double tmpArr[16];
    for (int z = 0; z < 16; z++)
      tmpArr[z] = 0;
    ...
    if (ffLegacySeeds) {
      /* ── FF legacy seeds: IndexBest.bin (15 dbl/seed @ rowNr*15) gives the
       * solution; IndexBestFull.bin (MaxNHKLS*2 dbl/seed @ rowNr*MaxNHKLS*2)
       * gives the matched spot IDs. Mirror the consolidated tmpArr layout:
       * [2..10]=Orient, [11..13]=Pos, [14]=NrExpected, [15]=NrObserved. ── */
      size_t rowNr = infoArr[thisRowNr * 5 + 0];
      ...
      if (inpF < 0)
        continue;
      double locArr[15];
      size_t offst1 = rowNr * 15 * sizeof(double);
      ssize_t rcA = pread(inpF, locArr, 15 * sizeof(double), offst1);
      close(inpF);
      if (rcA < (ssize_t)(15 * sizeof(double)) || locArr[14] == 0)
        continue; /* slot never written, or no good solution */
      nSpotsBest = (int)locArr[14];
      if (nSpotsBest > MaxNSpotsBest || nSpotsBest <= 0)
        continue;
      for (i = 0; i < 9; i++)
        tmpArr[i + 2] = locArr[i + 1];
      for (i = 0; i < 3; i++)
        tmpArr[i + 11] = locArr[i + 10];
      tmpArr[14] = locArr[13];
      tmpArr[15] = locArr[14];
      spotIDS = malloc(nSpotsBest * sizeof(*spotIDS));
      double *locArr2 = calloc((size_t)nSpotsBest * 2, sizeof(*locArr2));
      size_t offst2 = rowNr;
      offst2 *= MaxNHKLS;
      ...
      ssize_t rcB =
          (inpF2 >= 0)
              ? pread(inpF2, locArr2, (size_t)nSpotsBest * 2 * sizeof(double),
                      offst2)
              : -1;
      if (inpF2 >= 0)
        close(inpF2);
      if (rcB < (ssize_t)((size_t)nSpotsBest * 2 * sizeof(double))) {
        free(locArr2);
        free(spotIDS);
        continue;
      }
       ...
       * solutions 0..solIndex-1 to find this solution's IDs. */
      size_t idStartIdx = 0;
    ...
    // ════ Fit sequence (D8) ════
    // PF (nScans>1): orient → strain, position FIXED to the voxel-grid center.
    // FF (nScans<=1): orient → position-Ini → strain → position-Sec, mirroring
    // legacy FitPosOrStrainsOMP. Position uses the SPATIAL objective
    // (FitPosSec / FitErrorsPosSec, CalcNorm2 on detector y,z), distinct from
    // the angular metric the orient/strain stages use.
    int isFF = (gNumScans <= 1);

    // Position bounds (FF): seed ± Rsample/2, clamped to ±Rsample (y,z-lab) /
    // ±Hbeam/2 (along beam) — matching legacy FitPosOrStrainsOMP.
#define MIDAS_FG_POSBOUNDS(center, lo, hi)                                     \
  do {                                                                         \
    for (int _k = 0; _k < 3; _k++) {                                           \
      (lo)[_k] = (center)[_k] - Rsample / 2;                                   \
      (hi)[_k] = (center)[_k] + Rsample / 2;                                   \
    }                                                                          \
    if ((lo)[0] < -Rsample) (lo)[0] = -Rsample;                               \
    if ((lo)[1] < -Rsample) (lo)[1] = -Rsample;                               \
    if ((lo)[2] < -Hbeam / 2) (lo)[2] = -Hbeam / 2;                           \
    if ((hi)[0] > Rsample) (hi)[0] = Rsample;                                  \
    if ((hi)[1] > Rsample) (hi)[1] = Rsample;                                  \
    ...
    for (i = 0; i < 3; i++)
      posCur[i] = Pos0[i];
    if (isFF) {
      double posLb[3], posUb[3];
      MIDAS_FG_POSBOUNDS(Pos0, posLb, posUb);
      FitPosSec(Pos0, nSpotsComp, spotsYZONew, nhkls, hkls, Lsd, Wavelength,
                nOmeRanges, OmegaRanges, BoxSizes, MinEta, wedge, chi, posCur,
    ...
    for (i = 0; i < 3; i++)
      posFit[i] = posCur[i];
    if (isFF) {
      double posLb2[3], posUb2[3];
      MIDAS_FG_POSBOUNDS(posCur, posLb2, posUb2);
      FitPosSec(posCur, nSpotsComp, spotsYZONew, nhkls, hkls, Lsd, Wavelength,
...
#undef MIDAS_FG_POSBOUNDS
    ...
    printf("SpotID %6d, %3d spots, IniErr: %8.2f, Pos: %7.2f %7.2f %7.2f, "
           "Orient: %7.2f %7.2f %7.2f -> "
    ...
    if (ffLegacySeeds) {
      ...
      if (fdProcKey >= 0 && nSpotsComp > 0) {
        int *ProcessInfo = malloc(nSpotsComp * sizeof(int));
        for (i = 0; i < nSpotsComp; i++)
          ProcessInfo[i] = (int)SpotsComp[i][0];
        size_t pkOff = (size_t)MaxNHKLS * sizeof(int) * rowNrW;
        pwrite(fdProcKey, ProcessInfo, nSpotsComp * sizeof(int), pkOff);
        free(ProcessInfo);
      }
    }
