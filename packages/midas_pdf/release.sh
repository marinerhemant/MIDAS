#!/usr/bin/env bash
# Release a new version of midas-pdf.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-pdf-v${NEW_VERSION}"

echo "=== Releasing midas-pdf v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_pdf/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_pdf/__init__.py
rm -f pyproject.toml.bak midas_pdf/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_pdf/__init__.py | cut -d'"' -f2)
...
KMP_DUPLICATE_LIB_OK=TRUE python -m pytest tests/ -q --tb=short || {
    ...
    git checkout -- pyproject.toml midas_pdf/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_pdf/__init__.py"
    exit 0
fi
...
VERSION_FILES=(pyproject.toml midas_pdf/__init__.py)

git add -- "${VERSION_FILES[@]}"
if git diff --cached --quiet -- "${VERSION_FILES[@]}"; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-pdf: bump version to ${NEW_VERSION}" \
        -- "${VERSION_FILES[@]}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-pdf v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    echo "  PyPI: https://pypi.org/project/midas-pdf/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-pdf && \\"
    echo "          python -c 'import midas_pdf; print(midas_pdf.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-pdf v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo
