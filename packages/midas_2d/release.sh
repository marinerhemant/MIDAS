#!/usr/bin/env bash
# Release a new version of midas-2d.
#
# Usage:
...
cd "$PKG_DIR"
TAG="midas-2d-v${NEW_VERSION}"

echo "=== Releasing midas-2d v${NEW_VERSION} (mode: ${MODE}) ==="
echo
...
if ! git diff --quiet HEAD -- .; then
    echo "ERROR: uncommitted changes in packages/midas_2d/. Commit or stash first."
    git status -s -- .
    exit 1
fi
...
sed -i.bak "s/^__version__ = \".*\"/__version__ = \"${NEW_VERSION}\"/" midas_2d/__init__.py
rm -f pyproject.toml.bak midas_2d/__init__.py.bak

PYPROJ_VER=$(grep '^version = ' pyproject.toml | cut -d'"' -f2)
INIT_VER=$(grep '^__version__ = ' midas_2d/__init__.py | cut -d'"' -f2)
...
KMP_DUPLICATE_LIB_OK=TRUE python -m pytest tests/ -q --tb=short || {
    ...
    git checkout -- pyproject.toml midas_2d/__init__.py
    exit 1
}
...
if [ "$MODE" = "--dry-run" ]; then
    ...
    echo "  git checkout -- pyproject.toml midas_2d/__init__.py"
    exit 0
fi
...
VERSION_FILES=(pyproject.toml midas_2d/__init__.py)

git add -- "${VERSION_FILES[@]}"
if git diff --cached --quiet -- "${VERSION_FILES[@]}"; then
    echo "  Version was already at ${NEW_VERSION} on disk; skipping commit."
else
    git commit -m "midas-2d: bump version to ${NEW_VERSION}" \
        -- "${VERSION_FILES[@]}"
fi

echo "[5/7] Tagging as ${TAG}..."
git tag -a "$TAG" -m "midas-2d v${NEW_VERSION}"
...
if [ "$MODE" = "--publish" ]; then
    ...
    echo "  PyPI: https://pypi.org/project/midas-2d/${NEW_VERSION}/"
    echo "  Verify: pip install -U midas-2d && \\"
    echo "          python -c 'import midas_2d; print(midas_2d.__version__)'"
    exit 0
fi
...
echo "    --title 'midas-2d v${NEW_VERSION}' \\"
echo "    --generate-notes"
echo
