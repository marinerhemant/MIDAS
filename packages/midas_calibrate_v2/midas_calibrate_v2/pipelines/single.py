...
def autocalibrate(
    ...
    *,
    ...
    """
    v1_params.validate()
    # Sanity-check RhoD (µm equivalent of v1.RhoD pixels) vs detector geometry.
    from ..forward.sanity import check_rho_d_um
    rho_d_px = v1_params.RhoD if v1_params.RhoD > 0 else v1_params.MaxRingRad
    check_rho_d_um(
        RhoD_um=float(rho_d_px) * float(v1_params.pxY),
    ...
    for it in range(n_iter):
        ...
        # Compute mean strain at the converged unpacked dict.
        with torch.no_grad():
            r_final = residual_fn(unpacked)
            mean_strain_uE = float(r_final.abs().mean()) * 1e6
        ...
        with torch.no_grad():
            ...
            if abs_r.size >= 50:
                ...
                if int(keep.sum()) >= 50:
                    ...
                    post_strain = float(r_post.abs().mean()) * 1e6
                    if verbose:
