...
Notebooks 01–04 run on **CPU** with **synthetic** data — no external
datasets, no GPU. Notebook 05 is a **bring-your-own-data** real-data PF
recon (point it at a run directory; it is not part of the CI execute
sweep). Source of truth is `_build.py`; the `.ipynb` files are derived
artefacts. Set `$MIDAS_HOME` if the repo is not at `~/opt/MIDAS`.
...
python _build.py
# execute only the synthetic notebooks (05 needs a real dataset):
KMP_DUPLICATE_LIB_OK=TRUE jupyter nbconvert --to notebook --execute --inplace \
    0[1-4]_*.ipynb --ExecutePreprocessor.timeout=300
```
...
| `05_pf_real_data_recon.ipynb` | **Bring-your-own-data** real-data PF recon end-to-end: grain map (c-omp indexing → find_grains) → V-map (gauge + soft-attribution) → per-voxel orientation+strain (full3d loss + neighbour-corrected seeds) → lab-frame strain via `midas_stress`. Point the config cell at a run directory. Released packages only; no private scripts. | needs a dataset |
