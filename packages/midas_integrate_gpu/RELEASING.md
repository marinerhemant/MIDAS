# Releasing midas-integrate-gpu

How to cut a new release of `midas-integrate-gpu` to PyPI + GitHub.

## Quick reference

```bash
cd packages/midas_integrate_gpu
...
2. Verify tag `midas-integrate-gpu-v0.1.1` does not exist.
3. Verify `cmake`, C compiler, and NLopt headers are present locally.
4. Bump version in `pyproject.toml`, `midas_integrate_gpu/__init__.py`,
...
8. Create annotated git tag `midas-integrate-gpu-v0.1.1`.
...
gh release create midas-integrate-gpu-v0.1.1 dist/* \
    --title "midas-integrate-gpu v0.1.1" \
    --generate-notes
```
...
that `midas-integrate-gpu` links against, bump the minor version and update
the `midas-integrate-gpu>=X.Y,<X.Y+1` pin in `midas-integrate-gpu`'s
