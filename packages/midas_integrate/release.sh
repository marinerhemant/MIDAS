...
python -c "import midas_auto_calibrate, packaging.version as v; \
    assert v.parse(midas_auto_calibrate.__version__) >= v.parse('0.1.0'), \
    f'need midas-auto-calibrate >= 0.1.0, got {midas_auto_calibrate.__version__}'" \
    2>/dev/null || {
    echo "ERROR: midas-auto-calibrate >= 0.1.0 not installed in this env."
    echo "  pip install -e ../midas_auto_calibrate"
    exit 1
}
...
if [ "$MODE" = "--publish" ]; then
    ...
    echo "  https://pypi.org/project/midas-integrate/${NEW_VERSION}/"
