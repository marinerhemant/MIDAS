# Releasing midas-integrate

How to cut a new release of `midas-integrate` to PyPI + GitHub.

## Quick reference

```bash
cd packages/midas_integrate
...
2. Verify tag `midas-integrate-v0.1.1` does not exist.
3. Verify `cmake`, C compiler, and NLopt headers are present locally.
4. Bump version in `pyproject.toml`, `midas_integrate/__init__.py`,
...
8. Create annotated git tag `midas-integrate-v0.1.1`.
...
gh release create midas-integrate-v0.1.1 dist/* \
    --title "midas-integrate v0.1.1" \
    --generate-notes
```
...
the `midas-integrate>=X.Y,<X.Y+1` pin in `midas-integrate`'s
