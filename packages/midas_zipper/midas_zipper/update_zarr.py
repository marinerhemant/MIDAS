"""Zarr dataset updater (ported from utils/updateZarrDset.py).

Standalone, pip-installable. Pure Python + system ``zip``. Run via
``python -m midas_zipper.update_zarr`` or the ``midas-update-zarr`` script.
"""
