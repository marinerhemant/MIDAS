"""FF zarr-zip generation (ported from utils/ffGenerateZipRefactor.py).

Standalone, pip-installable. No MIDAS source-tree or C-binary dependency
(uses h5py / numpy / zarr / numcodecs / numba + system bzip2 only).
Run as ``python -m midas_zipper.ff_zip -resultFolder ... -paramFN ... -LayerNr ...``
or via the ``midas-ff-zip`` console script.
"""
...
# (MIDAS version banner removed — midas_zipper is a standalone pip package.)
