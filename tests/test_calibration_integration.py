...
Usage:
    python test_calibration_integration.py -nCPUs 4
    python test_calibration_integration.py -nCPUs 4 --calibration-only
    python test_calibration_integration.py -nCPUs 4 --keep-work-dir
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Calibration + Integration benchmark (merged test)')
    ...
    parser.add_argument('--calibration-only', action='store_true',
                        help='Run only the calibration validation (no integration)')
    parser.add_argument('-strainThreshold', type=float, default=50.0,
                        help='Maximum acceptable calibration MeanStrain in µε (default: 50)')
    ...
    print("=" * 70)
    print("  MIDAS Calibration + Integration Benchmark")
    print("=" * 70)
    print(f"  Calibration dir: {CALIB_DIR}")
    print(f"  CPUs: {args.nCPUs}")
    if args.calibration_only:
        print(f"  Mode: calibration-only (strain threshold: {args.strainThreshold} µε)")
    else:
    ...
    try:
        ...
        if args.skip_calibration:
            ...
            # Validate calibration MeanStrain (from test_ff_calibration)
            mean_strain = optimized.get('MeanStrain')
            if mean_strain is not None:
                passed = mean_strain <= args.strainThreshold
                reporter.record('calibration/MeanStrain', passed=passed,
                                max_diff=mean_strain,
                                failure_type='value_threshold' if not passed else '',
                                threshold=args.strainThreshold)
                if passed:
                    print(f"\n  ✅ Calibration MeanStrain {mean_strain:.2f} µε ≤ {args.strainThreshold} µε")
                else:
                    print(f"\n  ❌ Calibration MeanStrain {mean_strain:.2f} µε > {args.strainThreshold} µε")
            else:
                print("\n  ⚠️  MeanStrain not found in calibrant output")
                reporter.record('calibration/MeanStrain', passed=False,
                                failure_type='missing_in_output')

            if args.calibration_only:
                # Print refined parameters and exit
                for k in ['StdStrain', 'MedianStr', 'Q25', 'Q75', 'MinStrain',
                           'MaxStrain', 'NPoints']:
                    if k in optimized:
                        fmt = f"{int(optimized[k])}" if k == 'NPoints' else f"{optimized[k]:.2f}"
                        print(f"  {k}: {fmt}")
                reporter.summary()
