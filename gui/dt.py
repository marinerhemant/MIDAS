...
def getfn_1(fstem):
 return fstem+'/'+samplename+'_method1_FileNrs_'+str(StartNr)+'_'+str(EndNr)+'_'+str(Resulttype_m1)+'_Rad_'+str(Rad_m1)+'_pm_'+str(Rwidth)+'_Eta_'+str(Eta_m1)+'_pm_'+str(etaWidth)+'_size_'+str(nfile)+'x'+str(nframe)+'_filter_'+str(filterNr)+'_'+str(NrPixels_m1)+'x'+str(NrPixels_m1)+'_float32.bin'

def getfn_2(fstem):
 return fstem+'/'+samplename+'_FileNrs_'+str(StartNr)+'_'+str(EndNr)+'_'+str(Resulttype_m2)+'_Rad_'+str(Rad_m2)+'_pm_'+str(Rwidth)+'_Eta_'+str(Eta_m2)+'_pm_'+str(etaWidth)+'_size_'+str(nfile)+'x'+str(nframe)+'_recon_'+str(NrPixels_m2)+'x'+str(NrPixels_m2)+'.bin'
...
def readParams():
    ...
    global NrPixels_m1,StartNr,EndNr,samplename,nframe,Rad,Rwidth,Eta,etaWidth,nfile,filterNr
    ...
    for line in paramContents:
        ...
        if 'rads'==line.split()[0]:
         ...
         for radN in range(len(lineRadarray)-1):
          Rad.append((int(lineRadarray[1+radN])))
        ...
        if 'etas'==line.split()[0]:
         ...
         for etaN in range(len(lineEtaarray)-1):
          Eta.append(int(lineEtaarray[1+etaN]))
    ...
    NrPixels_m2=nfile
    cbox_Rad_1.config(value=Rad)
    cbox_Eta_1.config(value=Eta)
    cbox_Rad_2.config(value=Rad)
    cbox_Eta_2.config(value=Eta)
...
def func1(event):
    global Resulttype_m1
    Resulttype_m1=cbox_rs_1.get()

def func2(event):
    global Rad_m1
    Rad_m1=cbox_Rad_1.get()

def func3(event):
    global Eta_m1
    Eta_m1=cbox_Eta_1.get()

def func4(event):
    global Resulttype_m2
    Resulttype_m2=cbox_rs_2.get()

def func5(event):
    global Rad_m2
    Rad_m2=cbox_Rad_2.get()

def func6(event):
    global Eta_m2
    Eta_m2=cbox_Eta_2.get()
