...
# -*- coding: utf-8 -*-
"""
rietveld_refine.py — Automated Rietveld Refinement using GSASIIscriptable

Reads MIDAS .zarr.zip caked output and one or more CIF files, then performs
a staged Rietveld refinement using the GSAS-II scripting API.  The stages
...
log = logging.getLogger("rietveld_refine")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=textwrap.dedent("""\
            Automated Rietveld Refinement with GSAS-II on MIDAS caked data.

            Imports a MIDAS .zarr.zip file and one or more CIF files, then
            performs a staged Rietveld refinement on every histogram (lineout).
            When --nCPUs > 1, histograms are refined in parallel.
...
def main():
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("  Rietveld Refinement Complete")
    print("=" * 60)
