...
main(int argc, char *argv[])
{
    ...
    printf("Diffraction spots: %lu mb\n",(TotalDiffrSpots*(3*sizeof(double) + sizeof(*ObsSpotsInfo) ))/(1024*1024));
    ...
    for (i=0;i<NrOrientations;i++){
        fgets(line,1000,fo);
        sscanf(line,"%lf %lf %lf %lf %lf %lf %lf %lf %lf",&OrientationMatrix[i][0],
			&OrientationMatrix[i][1],&OrientationMatrix[i][2],
			&OrientationMatrix[i][3],&OrientationMatrix[i][4],
			&OrientationMatrix[i][5],&OrientationMatrix[i][6],
			&OrientationMatrix[i][7],&OrientationMatrix[i][8]);
    }
