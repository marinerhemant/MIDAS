...
main(int argc, char *argv[])
{
    ...
    while (crestr < NrOrientations){
		...
		sscanf(line,"%s %s %s %s %s %s %s %s %s %lf %lf %lf %lf %lf %lf",dummy,dummy,dummy,dummy,dummy,dummy,
			dummy,dummy,dummy,&LatCsRead[crestr][0],&LatCsRead[crestr][1],
			&LatCsRead[crestr][2],&LatCsRead[crestr][3],&LatCsRead[crestr][4],&LatCsRead[crestr][5]);
		crestr++;
	}
    ...
    if (OrientationGoodID>0){
        ...
        double Fractions, EulerIn[3], OrientIn[3][3], FracOut, EulerOutA, EulerOutB,EulerOutC,OMTemp[9],LatCThis[6];
        BestFrac = -1;
        for (i=0;i<OrientationGoodID;i++){
            for (j=0;j<9;j++){
                OMTemp[j] = OrientMatrix[i][j];
            }
            for (j=0;j<6;j++) LatCThis[j] = LatticeParameters[i][j];
            CorrectHKLsLatC(LatCThis,Lsd[0],Wavelength,hkls,Thetas,hklsIn,n_hkls);
