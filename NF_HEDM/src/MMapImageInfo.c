...
main(int argc, char *argv[])
{
    ...
    for (i=0;i<TotalDiffrSpots;i++){
        ...
        sscanf(line,"%lf %lf %lf",&SpotsMat[3*i],&SpotsMat[3*i+1],&SpotsMat[3*i+2]);
    }
    fclose(fd);
    double *OrientationMatrix;
    OrientationMatrix = malloc(NrOrientations*9*sizeof(*OrientationMatrix));
    printf("ObsSpotsInfo: %llu mb, NrSpots: %lu mb, SpotsMat: %lu mb and Orientation Matrix: %lu mb\n",
			SizeObsSpots*sizeof(*ObsSpotsInfo)/(1024*1024), NrOrientations*2*sizeof(*NrSpots)/(1024*1024),
			TotalDiffrSpots*3*sizeof(*SpotsMat)/(1024*1024), NrOrientations*9*sizeof(*OrientationMatrix)/(1024*1024));
    for (i=0;i<NrOrientations;i++){
        rx = fgets(line,1000,fo);
        sscanf(line,"%lf %lf %lf %lf %lf %lf %lf %lf %lf",&OrientationMatrix[9*i],
			&OrientationMatrix[9*i+1],&OrientationMatrix[9*i+2],
			&OrientationMatrix[9*i+3],&OrientationMatrix[9*i+4],
			&OrientationMatrix[9*i+5],&OrientationMatrix[9*i+6],
			&OrientationMatrix[9*i+7],&OrientationMatrix[9*i+8]);
    }
    fclose(fo);
    char SI[1024];
