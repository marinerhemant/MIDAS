...
FindInternalAngles(int nrIDs, int *IDs, int *IDsPerGrain,
				   ...
				   int StartingID, double *Radiuses,int SGNr, int Twin)
{
			...
			if (ThisID == ThisID2 && IDsChecked[j] == false){
				...
				if (fabs(ang) < 0.1){
					counter = FindInternalAngles(nrIDs,IDs,IDsPerGrain,NrIDsPerID,IDsChecked,
							OPs,ID_IA_Mat,counter,j,ThisID,Radiuses,SGNr,Twin);
					break;
				} else if (Twin==1){
				    if (fabs(ang - 60.0) < 0.1 && 
					 fabs(fabs(Axis[0])-fabs(Axis[1])) < 0.1 && 
					 fabs(fabs(Axis[2])-fabs(Axis[1])) < 0.1 &&
					 fabs(fabs(Axis[2])-fabs(Axis[0])) < 0.1){
	...
	for (i=0;i<nrIDs;i++){
		...
		if (IDsChecked[i] == false){
			...
			counten = FindInternalAngles(nrIDs,IDs,IDsPerGrain,NrIDsPerID,
				IDsChecked,OPs,ID_IA_MAT,counte,i,StartingID,Radiuses,SGNr,Twin);
			totcount+=counten;
