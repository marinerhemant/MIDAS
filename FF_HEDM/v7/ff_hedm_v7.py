...
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/PeaksFittingOMPZarr")+' '+outFStem+'.zip 0 1 '+str(numProcs),shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
...
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/MergeOverlappingPeaksAllZarr")+' '+outFStem+'.zip',shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
...
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/CalcRadiusAllZarr")+' '+outFStem+'.zip',shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
...
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/FitSetupZarr")+' '+outFStem+'.zip',shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
os.chdir(resultDir)
...
f = open(f'indexing_out.csv','w')
f_err = open(f'indexing_err.csv','w')
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/IndexerOMP")+' paramstest.txt 0 1 '+str(num_lines)+' '+str(numProcs),shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
print(f"Refining. Time till now: {time.time()-t0}")
f = open(f'refinement_out.csv','w')
f_err = open(f'refinement_err.csv','w')
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/FitPosOrStrainsOMP")+' paramstest.txt 0 1 '+str(num_lines)+' '+str(numProcs),shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
subprocess.call("rm -rf /dev/shm/*.bin",shell=True)
...
subprocess.call(os.path.expanduser("~/opt/MIDAS/FF_HEDM/bin/ProcessGrainsZarr")+' '+outFStem+'.zip',shell=True,env=env,stdout=f,stderr=f_err)
f.close()
f_err.close()
print(f"Making plots, condensing output. Time till now: {time.time()-t0}")
subprocess.call('python '+os.path.expanduser('~/opt/MIDAS/utils/plotFFSpots3d.py')+' -resultFolder .',shell=True)
subprocess.call('python '+os.path.expanduser('~/opt/MIDAS/utils/plotFFSpots3dGrains.py')+' -resultFolder .',shell=True)
subprocess.call('python '+os.path.expanduser('~/opt/MIDAS/utils/plotGrains3d.py')+' -resultFolder .',shell=True)
