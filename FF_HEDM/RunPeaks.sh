...
swift -sites.file ${PFDIR}sites$2.xml -tc.file ${PFDIR}tc.data -config ${PFDIR}cf ${PFDIR}RunPeaks.swift -paramsfile=${ParamsFile} -ringnr=${RingNr} -startnr=${StartNr} -endnr=${EndNr}
