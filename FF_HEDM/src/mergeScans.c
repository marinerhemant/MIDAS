...
int main(int argc, char *argv[]) {
  ...
  if (argc < 11) {
    printf("Usage: %s FolderName FileStem Extension(with.) nScans nScansMerge "
           "nFramesMerge nPixelsY nPixelsZ nFrames nCPUs "
           "(optional)omegaFileName (optional)omegaStep\n",
           argv[0]);
    return 1;
  }
  char *folder = argv[1];
  ...
  int omegaNr = 0;
  if (argc == 13) {
    printf("Omega file was provided, it will rotate scans to start at -180 "
           "degrees.\n");
    omegaFile = argv[11];
...
#pragma omp parallel for num_threads(nCPUs) private(jobNr) schedule(dynamic)
  for (jobNr = 0; jobNr < nJobs; jobNr++) {
    char outFN[1024];
    sprintf(outFN, "%s/merged_scans/scanNr_%d_%dx%dx%d.bin", folder, jobNr,
            nFramesOut, nPxZ, nPxY);
    ...
    for (fileNr = startScanNr; fileNr < endScanNr; fileNr++) {
      ...
      for (frameNr = 0; frameNr < nFrames; frameNr++) {
        ...
        if (omegaNr > 0) {
          ...
          if (currentOmega < -180.0)
            printf("%lf\n", currentOmega);
          double recalcFrameNr =
              (180.0 + currentOmega) / (omegaStep * nFramesMerge);
          frameToPut = (int)floor(recalcFrameNr);
        } else
          frameToPut = frameNr / nFramesMerge;
        dsz = blosc1_decompress(&allData[sizeArr[frameNr * 2 + 1]], rawImage,
                                dsz);
        ...
        for (cntr = 0; cntr < dsz / 2; cntr++) {
          ...
          if (outArr[offset] > maxVal)
            maxVal = outArr[offset];
        }
      }
      t_1 = omp_get_wtime();
      free(finfo);
    ...
    for (iterator = 0; iterator < szarr; iterator++)
      outArrWrite[iterator] = (uint16_t)65536 * outArr[iterator] / maxVal;
    // Write file out.
