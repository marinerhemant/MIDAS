	...
	void* f_data_trial)
{
	...
	double Etas[4], Eta, RNorm, Rad, EtaT, DistortFunc, Rcorr, YCorrected[i], ZCorrected[i];
	for (i=0;i<4;i++){
		Yc = -(Ys[i]-Ycen)*px;
		Zc =  (Zs[i]-Zcen)*px;
...
void FitTX(double Lsd, double Ycen, double Zcen, double p0, double p1,
		   double p2, double MaxRad, double ty, double tz, double px, double Ys[4],
		   double Zs[4], double txIn, double *TxFit){
...
int main (int argc, char *argv[]){
    ...
    char *str, dummy[1000];
    int LowNr;
    double Ycen, Zcen, px, Lsd, ty, tz, p0, p1, p2, MaxRad, txIn, Ys[4], Zs[4], Y[4], Z[4], Omegas[4];
    int cntr=0;
