  ...
  *MeanDiff = minf / (OBJ_FUNC_SCALE * nIndices);
  if (nPanels > 0) {
    ...
    if (tolShifts > EPS && nPanels > 1) {
      ...
      // Calculate deviations for all points to get accurate MeanDiff (excluding
      // off-panel points) This logic runs regardless of outlierFactor to ensure
      // consistency.
      {
        double txr, tyr, tzr;
    ...
    static inline void CorrectTiltSpatialDistortion(
        ...
        double *StdDiff, double outlierFactor) {
      double txr, tyr, tzr;
      txr = deg2rad * tx;
      tyr = deg2rad * ty;
      tzr = deg2rad * tz;
        ...
        if (TransOpt[k] == 1) {
          for (l = 0; l < NrPixels; l++)
            for (m = 0; m < NrPixels; m++)
              ImageTemp2[l][m] =
                  ImageTemp1[l][NrPixels - m - 1]; // Inverting Y.
        } else if (TransOpt[k] == 2) {
          for (l = 0; l < NrPixels; l++)
            for (m = 0; m < NrPixels; m++)
              ImageTemp2[l][m] =
                  ImageTemp1[NrPixels - l - 1][m]; // Inverting Z.
    ...
    int fileReader(FILE * f, char fn[], int dType, int NrPixels,
      ...
      if (dType == 1) {
        ...
        status_n = H5Dread(dataset, H5T_STD_U16LE, H5S_ALL, H5S_ALL,
                           H5P_DEFAULT, data);
    ...
    int main(int argc, char *argv[]) {
      ...
      if (NPanelsY > 0 && NPanelsZ > 0) {
        if (GeneratePanels(NPanelsY, NPanelsZ, PanelSizeY, PanelSizeZ,
                           PanelGapsY, PanelGapsZ, &panels, &nPanels) != 0) {
      ...
      AverageDark =
          calloc(NrPixels * NrPixels, sizeof(*AverageDark));   // Squared.
      ...
      for (a = StartNr; a <= EndNr; a++) {
        ...
        if (dType != 8) {
          ...
          Skip = HeadSize;
          printf(
              "Reading calibrant file: %s, nFrames: %d %d %d, skipping first "
              "%ld bytes.\n",
          ...
          for (j = 0; j < nFrames; j++) {
            ...
            for (k = 0; k < (NrPixels * NrPixels); k++) {
              Average[k] += (double)(Image2[k]) -
                            AverageDark[k]; // In reality this is sum
            }
          }
          TotFrames += nFrames;
          fclose(fp);
        ...
        Car2Pol(n_hkls, nEtaBins, NrPixels, NrPixels, ybc, zbc, px, R, Eta,
                ...
                Lsd);
        double *RMean, *EtaMean, *IdealR, *IdealTtheta, *IdealRmins,
            *IdealRmaxs;
        ...
        double ty, tz, LsdFit, ybcFit, zbcFit, p0, p1, p2, p3, MeanDiff, *Yc,
            *Zc, *EtaIns, *RadIns, *DiffIns, StdDiff;
        ...
        CorrectTiltSpatialDistortion(nIndices, MaxRingRad, Yc, Zc, IdealTtheta,
                                     ...
                                     &StdDiff, outlierFactor);
        NrCalls = 0;
        FitTiltBCLsd(nIndices, Yc, Zc, IdealTtheta, Lsd, MaxRingRad, ybc, zbc,
