...
static void CalculateAndPrintUncertainties(unsigned n, double *x, void *f_data,
  ...
  // Diagonal elements
  for (int i = 0; i < n; i++) {
    double h = (fabs(x[i]) > 1e-8) ? fabs(x[i]) * h_rel
                                   : 1e-6; // Handle near-zero params
    memcpy(xt, x, n * sizeof(double));

    xt[i] = x[i] + h;
    double f_plus = problem_function_SSD(n, xt, NULL, f_data);

    xt[i] = x[i] - h;
    double f_minus = problem_function_SSD(n, xt, NULL, f_data);
  ...
  for (int i = 0; i < n; i++) {
    double hi = (fabs(x[i]) > 1e-8) ? fabs(x[i]) * h_rel : 1e-6;
    for (int j = i + 1; j < n; j++) {
      double hj = (fabs(x[j]) > 1e-8) ? fabs(x[j]) * h_rel : 1e-6;

      memcpy(xt, x, n * sizeof(double));
      xt[i] = x[i] + hi;
      xt[j] = x[j] + hj;
      double f_pp = problem_function_SSD(n, xt, NULL, f_data);

      xt[i] = x[i] + hi;
      xt[j] = x[j] - hj;
      double f_pm = problem_function_SSD(n, xt, NULL, f_data);

      xt[i] = x[i] - hi;
      xt[j] = x[j] + hj;
      double f_mp = problem_function_SSD(n, xt, NULL, f_data);

      xt[i] = x[i] - hi;
      xt[j] = x[j] - hj;
      double f_mm = problem_function_SSD(n, xt, NULL, f_data);
  ...
  // Invert Hessian - Cast to double** for MatrixInvert since pixelvalue is
  // double
  if (MatrixInvert((double **)H, n, (double **)HInv) != 0) {
    printf("Hessian inversion failed (singular matrix). cannot estimate "
           "uncertainties.\n");
  } else {
    printf(
        "\n******************* Parameter Uncertainties *******************\n");
    // Standard approximation: Covariance = 2 * H_SSD^-1 * sigma^2
    // where H_SSD is Hessian of SSD.

    for (int i = 0; i < n; i++) {
      double var_i = 2.0 * HInv[i][i] * sigma2;
      double std_i = (var_i > 0) ? sqrt(var_i) : 0.0;
      // Print with appropriate labels
      char label[64];
      if (i == 0)
        sprintf(label, "Lsd");
      else if (i == 1)
        ...
        sprintf(label, "ShiftParam_%d", i - 9);

      printf("%-16s: %0.12f +/- %0.12f\n", label, x[i], std_i);
    }
    printf(
        "***************************************************************\n\n");
  }
