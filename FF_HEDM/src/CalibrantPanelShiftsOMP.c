	...
	void* f_data_trial)
{
	...
	for (i=0;i<nIndices;i++){
        ...
        // In the table below, the numbers are pixel numbers
        // Y: 1st panel [0-243), 2nd panel [244-487), 3rd panel [494-737), 4th panel [738-981), 5th panel [988-1231), 6th panel [1232-1475)
        // Z: Total panels 8, separated by 17 pixels each.
        // Z: 1st panel [0-195), 2nd panel [212-407), 3rd panel [424-619), 4th panel [636-831), 5th panel [848-1043), 6th panel [1060-1255), 7th panel [1272-1467), 8th panel [1484-1679)
        // YMean and ZMean should first be convert to integers, these already pixel coordinates. Then we use the table above to figure out YPanel and ZPanel, which will give us panelNumber
        int Ydet = (int) round(YMean[i]);
        int Zdet = (int) round(ZMean[i]);
        int panelY = -1;
        for (j=0;j<6;j++){
            if (Ydet >= yMins[j] && Ydet < yMaxs[j]){
                panelY = j;
                break;
            }
        }
        ...
        int panelNumber = panelY * 8 + panelZ;
        if (n>9){
            dY = x[9+panelNumber*2 + 0];
            dZ = x[9+panelNumber*2 + 1];
				  ...
				  double tolBC, double tolP, double tolP0, double tolP1, double tolP2, double tolP3, double tolShifts, double px)
{
    ...
    int nPanels;
    if (tolShifts > EPS) {
        // Instead of n=9, we need n=9+nPanels*2
        nPanels = 8*6;
        n += nPanels*2; 
    }
    ...
    if (tolShifts > EPS) {
        for (int i=0; i<nPanels; i++) {
            ...
            // dz for each panel
            x[9 + i*2 + 1] = 0;
            xl[9 + i*2 + 1] = -tolShifts;
            xu[9 + i*2 + 1] = tolShifts;
        }
    }
...
int main(int argc, char *argv[])
{
	...
	for (a=StartNr;a<=EndNr;a++){
		...
		FitTiltBCLsd(nIndices,Yc,Zc,IdealTtheta,Lsd,MaxRingRad,ybc,zbc,tx,tyin,tzin,p0in,p1in,p2in,p3in,&ty,&tz,&LsdFit,&ybcFit,&zbcFit,
					 &p0,&p1,&p2,&p3,&MeanDiff,tolTilts,tolLsd,tolBC,tolP,tolP0,tolP1,tolP2,tolP3,tolShifts,px);
