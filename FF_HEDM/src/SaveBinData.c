...
int main(int arc, char *argv[]) {
  ...
  while (fgets(aline, 4096, ObsSpotsFile) != NULL) {
    sscanf(aline, "%lf %lf %lf %lf %lf %lf %lf %lf", &ObsSpots[nSpots][0],
           &ObsSpots[nSpots][1], &ObsSpots[nSpots][2], &ObsSpots[nSpots][3],
           &ObsSpots[nSpots][4], &ObsSpots[nSpots][5], &ObsSpots[nSpots][6],
           &ObsSpots[nSpots][7]);
    nSpots++;
  }
  ...
  while (fgets(aline, 4096, AllSpotsFile) != NULL) {
    sscanf(aline, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
           &AllSpots[countr][0], &AllSpots[countr][1], &AllSpots[countr][2],
           &AllSpots[countr][3], &AllSpots[countr][4], &AllSpots[countr][5],
           &AllSpots[countr][6], &AllSpots[countr][7], &AllSpots[countr][8],
           &AllSpots[countr][9], &AllSpots[countr][10], &AllSpots[countr][11],
           &AllSpots[countr][12], &AllSpots[countr][13]);
    countr++;
  }
  if (nSpots != countr) {
  ...
  for (rowno = 0; rowno < nSpots; rowno++) {
    ...
    int iRing = ringnr - 1;
    if ((iRing < 0) || (iRing > n_ring_bins - 1))
      continue;
