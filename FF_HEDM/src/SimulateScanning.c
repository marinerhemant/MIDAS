...
int main (int argc, char *argv[]){
	...
	while(fgets(aline,4096,fileParam)!=NULL){
		...
		if (strncmp(aline,"nLayers",strlen("nLayers"))==0){ // This is actually the number of scans.
	...
	//		SpotsList: 14*nSpots [0:y,1:z,2:omega,6:eta,7:2theta] Binary format
	//		IDsHash [for upto maxRingNr for each spotNr, positionNr, ringNr, startRowNr+1, endRowNr+1] csv format
	// What we need: voxelList, beamPositions, LatticeParameter, Wavelength, HKLs, ringsToUse, nRings,
	...
	for (voxelNr=0;voxelNr<nVoxels;voxelNr++){
		...
		thisPosition[2] = 0;
		nSpots = CalcDiffractionSpots(Lsd,Wavelength,thisPosition,LatC,Euler,nhkls,hkls,localSpotInfo,1);
		for (spotNr=0;spotNr<nSpots;spotNr++){
			...
			for (positionNr=0;positionNr<nBeamPositions;positionNr++){
				...
				voxelFraction = IntensityFraction(voxelLen,thisBeamPosition,beamFWHM,thisPosition,thisOmega);
				if (voxelFraction > 0){
					rowNr = positionNr*(nhkls+2)*2*14 + bestHKLNr*14;
					SpotInfo[rowNr+0] += localSpotInfo[spotNr*9+7]*voxelFraction;//y
					SpotInfo[rowNr+1] += localSpotInfo[spotNr*9+8]*voxelFraction;//z
					SpotInfo[rowNr+2] += localSpotInfo[spotNr*9+4]*voxelFraction;//omega
					SpotInfo[rowNr+3] += voxelFraction;//voxelFraction
					thisEta = CalcEta(localSpotInfo[spotNr*9+7],localSpotInfo[spotNr*9+8]);
					SpotInfo[rowNr+4] = localSpotInfo[spotNr*9+5]; // ringNr
					SpotInfo[rowNr+6] += thisEta*voxelFraction; // Eta
					SpotInfo[rowNr+7] += Calc2Theta(localSpotInfo[spotNr*9+7],localSpotInfo[spotNr*9+8],Lsd)*voxelFraction; // 2Theta
				}
			}
		}
	}
	...
	for (positionNr=0;positionNr<nBeamPositions;positionNr++){
		...
		for (bestHKLNr=0;bestHKLNr<(nhkls+2)*2;bestHKLNr++){
			...
			if (SpotInfo[spotPosNr+3] > 0){
				...
				if (IDsInfo[idsrownr+2] == 0) IDsInfo[idsrownr+2] = nrFilled;
	...
	fclose(fb);
	fid = fopen("SimIDsHash.bin","w");
	for (i=0;i<nRings*nBeamPositions;i++)
		fprintf(fid,"%d %d %d %d %d\n",IDsInfo[i*5+0],IDsInfo[i*5+1],IDsInfo[i*5+2],IDsInfo[i*5+3],IDsInfo[i*5+4]);
	fclose(fid);
}
