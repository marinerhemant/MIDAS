...
//
//
// TODO: Rectangular detector: experimental support is included now, flood will
// not read properly, also, not all image transforms are supported for
// transforms (eg. transpose might give strange values) 		Read edf
...
FindRegionalMaxima(double *z, int *PixelPositions, int NrPixelsThisRegion,
                   int *MaximaPositions, double *MaximaValues, int *IsSaturated,
...
void main(int argc, char *argv[]) {
  ...
  // NFrames must be total number of frames, it will start from 0 and end at
  // nFrames
  ...
  char *str, dummy[1000], Folder[1024], FileStem[1024], *TmpFolder,
      darkcurrentfilename[1024], floodfilename[1024], Ext[1024],
      RawFolder[1024];
  TmpFolder = "Temp";
  int LowNr;
  double bc = 1, Ycen = 1024, Zcen = 1024, IntSat = 16000, OmegaStep = 0.25,
         OmegaFirstFile = -180, Lsd = 1000000, px = 200, Width = 1000,
         Wavelength = 0.189714, MaxRingRad = 204800;
  ...
  while (fgets(aline, 1000, fileParam) != NULL) {
    ...
    LowNr = strncmp(aline, str, strlen(str));
    if (LowNr == 0) {
      sscanf(aline, "%s %d %lf %lf", dummy, &FrameNrOmeChange, &OmegaMissing,
             &MisDir);
      continue;
    }
    ...
    LowNr = strncmp(aline, str, strlen(str));
    if (LowNr == 0) {
      sscanf(aline, "%s %lf %lf", dummy, &FileOmegaOmeStep[fnr][0],
             &FileOmegaOmeStep[fnr][1]);
      fnr++;
      continue;
    }
    str = "RingThresh ";
  ...
  if (darkfile == NULL) {
    printf("Could not read the dark file. Using no dark subtraction.\n");
  } else {
...
#pragma omp parallel for num_threads(numProcs) private(FileNr) schedule(dynamic)
  for (FileNr = startFileNr; FileNr < endFileNr; FileNr++) {
    ...
    asym_idxoffset = NrPxY;
    asym_idxoffset *= NrPxZ;
...
#pragma omp critical
    ...
    if (fnr == 0) {
      if (FileNr - StartNr + 1 < FrameNrOmeChange) {
        ...
        Omega = OmegaFirstFile + ((FileNr - StartNr + 1) * OmegaStep) +
                MisDir * OmegaMissing * Nadditions;
      }
    } else {
      Omega = FileOmegaOmeStep[ReadFileNr - StartFileNr][0] +
              FramesToSkip * FileOmegaOmeStep[ReadFileNr - StartFileNr][1];
    }
    char OutFile[1024];
    sprintf(OutFile, "%s/%s_%0*d_PS.csv", OutFolderName, FileStem, Padding,
            FileNr + StartNr);
    FILE *outfilewrite;
    outfilewrite = fopen(OutFile, "w");
    fprintf(outfilewrite, "SpotID IntegratedIntensity Omega(degrees) YCen(px) "
                          "ZCen(px) IMax Radius(px) Eta(degrees) SigmaR "
                          "SigmaEta NrPixels TotalNrPixelsInPeakRegion nPeaks "
                          "maxY maxZ diffY diffZ rawIMax returnCode\n");
    int KeepSpots = 0;
    for (i = 0; i < nOmeRanges; i++) {
      if (Omega >= OmegaRanges[i][0] && Omega <= OmegaRanges[i][1])
        KeepSpots = 1;
    }
    if (KeepSpots == 0) {
      continue;
    ...
    // We can just calculate where to be using nF-FramesToSkip, this is needed
    // because FrameNr is now starting from 0, not 1.
    ...
    for (RegNr = 1; RegNr <= NrOfReg; RegNr++) {
      ...
      nPeaks = FindRegionalMaxima(z, UsefulPixels, NrPixelsThisRegion,
                                  MaximaPositions, MaximaValues, &IsSaturated,
                                  IntSat, NrPixels);
      ...
      int rc = Fit2DPeaks(nPeaks, NrPixelsThisRegion, z, UsefulPixels,
                          MaximaValues, MaximaPositions, IntegratedIntensity,
                          IMAX, YCEN, ZCEN, Rads, Etass, Ycen, Zcen, Thresh,
                          NrPx, OtherInfo, NrPixels);
      for (i = 0; i < nPeaks; i++) {
        fprintf(outfilewrite, "%d %f %f %f %f %f %f %f ", (SpotIDStart + i),
                IntegratedIntensity[i], Omega, -YCEN[i] + Ycen, ZCEN[i] + Zcen,
                IMAX[i], Rads[i], Etass[i]);
        for (j = 0; j < 2; j++)
          fprintf(outfilewrite, "%f ", OtherInfo[2 * i + j]);
        fprintf(outfilewrite, "%d %d %d %d %d %f %f %f %d\n", NrPx[i],
                NrPixelsThisRegion, nPeaks, MaximaPositions[i * 2 + 0],
