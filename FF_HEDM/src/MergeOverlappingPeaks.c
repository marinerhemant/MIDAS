...
static inline int ReadSortFiles(char OutFolderName[1024], char FileStem[1024],
                                int FileNr, int RingNr, int Padding,
                                double **SortedMatrix) {
  char aline[1000], dummy[1000];
  char InFile[1024];
  sprintf(InFile, "%s/%s_%0*d_%d_PS.csv", OutFolderName, FileStem, Padding,
          FileNr, RingNr);
  FILE *infileread;
...
int main(int argc, char *argv[]) {
  ...
  nSpots =
      ReadSortFiles(OutFolderName, FileStem, FileNr, RingNr, Padding, NewIDs);
  for (i = 0; i < nSpots; i++) {
  ...
  if (e == 0)
    return 1;
  sprintf(OutFileName,
          "%s/PeakSearch/%s/Result_StartNr_%d_EndNr_%d_RingNr_%d.csv", Folder,
          FileStem, StartNr, EndNr, RingNr);
  FILE *OutFile;
  ...
  if (StartNr == EndNr) { // If there is only one file.
    ...
    for (FileNr = (StartNr + 1); FileNr <= EndNr; FileNr++) {
      nSpotsNew = ReadSortFiles(OutFolderName, FileStem, FileNr, RingNr,
                                Padding, NewIDs);
      fflush(stdout);
      for (i = 0; i < nSpots; i++) {
        minLen = 10000000;
