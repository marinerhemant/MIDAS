...
// v5.0: - Can read a hkl file and work for any crystal structure.
//
...
//            for (i = 0 ;  i < 9 ; i ++) { OrientMatrix[i/3][i%3] =  OrMat[or][i]; }
      ...
      if (fracMatches > 1 || fracMatches < 0 || (int)bestnTspotsIsp == 0 || (int)bestnMatchesIsp == -1 || bestMatchFound == 0){
		  ...
		  /*sprintf(ffn2,"%s/BestPos_%09d.csv",Params.OutputFolder,SpotIDs);
		  printf("%s %d %f %d\n",ffn2,SpotID2,SpotID, SpotIDs);
		  FILE *freh;
		  freh = fopen (ffn2,"w");
		  fprintf(freh,"%d\n",(int)SpotID2);
		  fclose(freh);*/
          ...
          for ( iOme0 = iOmeMin ; iOme0 <= iOmeMax ; iOme0++) {
            ...
            if ( iSpot >= maxnspot ) { 
               ...
               maxndata[iRing][iEta][iOme] = maxnspot;
      				...
      				if (TotNumberOfBins == 20) printf("%d %d %d %d \n",iRing,iEta,iOme,iSpot);
...
main(int argc, char *argv[]){
   printf("\n\n\t\tIndexer v4.0\nContact hsharma@anl.gov in case of questions about the SHORT3DXRD project.\n\n");
   ...
   // Read hkls
	...
	while (fgets(aline,1000,hklf)!=NULL){
		sscanf(aline, "%s %s %s %lf %d %lf %lf %lf %lf %s %lf",dummy,dummy,dummy,&Ds,&Rnr,&hc,&kc,&lc,&tht,dummy,&RRd);
		for (i=0;i<Params.NrOfRings;i++){
