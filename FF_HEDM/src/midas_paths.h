...
static int get_midas_bin_directory(char *buffer, size_t buffer_size) {
  ...
  if (get_executable_directory(exe_dir, sizeof(exe_dir)) != 0) {
    // Fallback removed.
    // If we can't find the executable path and MIDAS_HOME isn't set, we can't
    // guess. Proceeding to try relative path logic if get_executable_directory
    // somehow partially worked or just return failure if we really have no
    // information.

    // Actually, if get_executable_directory failed, we have no anchor.
    fprintf(stderr, "Error: Could not determine MIDAS bin directory. Please "
                    "set MIDAS_HOME environment variable.\n");
    return -1;
  }
