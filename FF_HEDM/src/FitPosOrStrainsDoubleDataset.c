...
static inline void CalcAngleErrors(
    ...
    double **SpList, double *Error, int *nSpotsComp, int notIniRun,
    double offsetX, double offsetY, double offsetZ, double offsetOmega)
...
static inline double FitErrorsOrientStrains(
    double x[9], int nSpotsComp, double **spotsYZO, double **spotsYZO2,
...
static inline double FitErrorsPosSec(
    ...
    double Strains[6], double offsets[4]) {
  int i, j;
  int nrMatchedIndexer = nSpotsComp;
  double LatC[6];
  for (i = 0; i < 6; i++)
...
static double problem_function_PosIni(unsigned n, const double *x, double *grad,
  ...
  for (i = 0; i < nSpotsComp; i++) {
    for (j = 0; j < 9; j++) {
      ...
      if (j < 8)
        spotsYZO2[i][j] = f_data->spotsYZO2[i][j];
    }
  }
  double offsets[4];
  for (i = 0; i < 4; i++)
  ...
  for (i = 0; i < n; i++)
    XIn[i] = x[i];
  double error =
      FitErrorsPosT(XIn, nSpotsComp, spotsYZO, spotsYZO2, nhkls, f_data->hkls,
...
static double problem_function_OrientIni(unsigned n, const double *x,
  ...
  for (i = 0; i < n; i++)
    XIn[i] = x[i];
  double error = FitErrorsOrientStrains(XIn, nSpotsComp, spotsYZO, spotsYZO2,
                                        nhkls, f_data->hkls, Lsd, Wavelength,
                                        nOmeRanges, OmegaRanges, BoxSizes,
                                        MinEta, wedge, chi, Pos, offsets);
  FreeMemMatrix(spotsYZO, nSpotsComp);
  FreeMemMatrix(spotsYZO2, nSpotsComp);
  return error;
}
...
static double problem_function_StrainIni(unsigned n, const double *x,
  ...
  for (i = 0; i < n; i++)
    XIn[i] = x[i];
  double error =
      FitErrorsStrains(XIn, nSpotsComp, spotsYZO, spotsYZO2, nhkls,
                       f_data->hkls, Lsd, Wavelength, nOmeRanges, OmegaRanges,
                       BoxSizes, MinEta, wedge, chi, Pos, Orient, offsets);
  FreeMemMatrix(spotsYZO, nSpotsComp);
  FreeMemMatrix(spotsYZO2, nSpotsComp);
  return error;
}
...
static double problem_function_Pos(unsigned n, const double *x, double *grad,
  ...
  for (i = 0; i < n; i++)
    XIn[i] = x[i];
  double error =
      FitErrorsPosSec(XIn, nSpotsComp, spotsYZO, spotsYZO2, nhkls, f_data->hkls,
                      Lsd, Wavelength, nOmeRanges, OmegaRanges, BoxSizes,
                      MinEta, wedge, chi, Orient, Strains, offsets);
  FreeMemMatrix(spotsYZO, nSpotsComp);
  FreeMemMatrix(spotsYZO2, nSpotsComp);
  return error;
}
...
#pragma omp critical
      ...
      CalcAngleErrors(nSpotsYZO, nhkls, nOmeRanges, Ini, spotsYZO, spotsYZO2,
                      ...
                      wedge, chi, SpotsComp, Splist, ErrorIni, &nSpotsComp, 0,
                      0.0, 0.0, 0.0, 0.0);
      double **spotsYZONew;
      ...
      FitPositionIni(X0, nSpotsComp, spotsYZONew, spotsYZO2, nhkls, hkls, Lsd,
      ...
      CalcAngleErrors(nSpotsComp, nhkls, nOmeRanges, XFit, spotsYZONew,
                      spotsYZO2, hkls, Lsd, Wavelength, OmegaRanges, BoxSizes,
                      MinEta, wedge, chi, SpotsComp, Splist, ErrorInt1,
      ...
      FitOrientIni(X0_2, nSpotsComp, spotsYZONew, spotsYZO2, nhkls, hkls, Lsd,
                   Wavelength, nOmeRanges, OmegaRanges, BoxSizes, MinEta, wedge,
                   chi, XFit2, lb2, ub2, PosFitOrientIn, offsets);
      double UseXFit[12];
      for (i = 0; i < 3; i++)
        UseXFit[i] = XFit[i];
      for (i = 0; i < 3; i++)
      ...
      CalcAngleErrors(nSpotsComp, nhkls, nOmeRanges, UseXFit, spotsYZONew,
                      spotsYZO2, hkls, Lsd, Wavelength, OmegaRanges, BoxSizes,
                      MinEta, wedge, chi, SpotsComp, Splist, ErrorInt2,
                      &nSpotsComp, 1, offsetX, offsetY, offsetZ, offsetOmega);
      for (i = 0; i < nSpotsComp; i++)
        for (j = 0; j < 9; j++)
          spotsYZONew[i][j] = Splist[i][j];
      ...
      FitStrainIni(X0_3, nSpotsComp, spotsYZONew, spotsYZO2, nhkls, hkls, Lsd,
                   Wavelength, nOmeRanges, OmegaRanges, BoxSizes, MinEta, wedge,
                   chi, XFit3, lb3, ub3, PosFitOrientIn, OrientFitIn, offsets);
      double UseXFit2[12];
      for (i = 0; i < 3; i++)
        UseXFit2[i] = XFit[i];
      for (i = 0; i < 3; i++)
      ...
      CalcAngleErrors(nSpotsComp, nhkls, nOmeRanges, UseXFit2, spotsYZONew,
                      spotsYZO2, hkls, Lsd, Wavelength, OmegaRanges, BoxSizes,
                      MinEta, wedge, chi, SpotsComp, Splist, ErrorInt3,
                      &nSpotsComp, 1, offsetX, offsetY, offsetZ, offsetOmega);
      for (i = 0; i < nSpotsComp; i++)
        for (j = 0; j < 9; j++)
          spotsYZONew[i][j] = Splist[i][j];
      ...
      FitPosSec(X0_4, nSpotsComp, spotsYZONew, spotsYZO2, nhkls, hkls, Lsd,
                Wavelength, nOmeRanges, OmegaRanges, BoxSizes, MinEta, wedge,
      ...
      CalcAngleErrors(nSpotsComp, nhkls, nOmeRanges, FinalResult, spotsYZONew,
                      spotsYZO2, hkls, Lsd, Wavelength, OmegaRanges, BoxSizes,
                      MinEta, wedge, chi, SpotsComp, Splist, ErrorFin,
                      &nSpotsComp, 1, offsetX, offsetY, offsetZ, offsetOmega);
