...
//
// OpenMP-parallelized version with a command-line argument to set the core
// count. Maps diffraction spots by converting them to g-vectors and finding the
...
void convert_to_g_vector(double y, double z, double omega, double distance,
...
int main(int argc, char *argv[]) {
  if (argc != 5) {
    ...
    fprintf(stderr, "  - target_folder: Contains the reference dataset and its "
                    "hash table.\n");
    fprintf(stderr, "  - offset_omega: Offset in Omega to map dataset 2 to "
                    "dataset 1 (micrometers).\n");
    fprintf(stderr,
            "  - num_cores: The number of CPU cores to use for processing.\n");
    return 1;
  }
  ...
  if (num_cores <= 0) {
    fprintf(
        stderr,
        "Warning: Invalid number of cores '%s'. Using all available cores.\n",
        argv[4]);
  } else {
  ...
  snprintf(extra_info_path, sizeof(extra_info_path), "%s/ExtraInfo.bin",
           source_folder);
  snprintf(output_path, sizeof(output_path), "%s/mapDatasets.txt",
           source_folder);

  AppParams params = {0};
  if (read_parameters(param_path, &params) != 0)
    return 1;
  ...
  for (int i = 0; i < params.num_ring_numbers; ++i) {
    if (params.ring_numbers[i] > highest_ring_no)
      highest_ring_no = params.ring_numbers[i];
  }
  int n_ring_bins = highest_ring_no;
  ...
  long n_spots =
      read_binary_file(spots_path, (void **)&spots_mat, sizeof(double)) /
      N_COL_OBSSPOTS;
  long n_source_points =
      read_binary_file(extra_info_path, (void **)&extra_mat, sizeof(double)) /
      N_COL_EXTRAINFO;
  long ndata_len =
      read_binary_file(ndata_path, (void **)&ndata_store, sizeof(int));
  long data_len =
      read_binary_file(data_path, (void **)&data_store, sizeof(int));

  // **FIX #1:** Added ndata_len and data_len to the check
  if (n_spots <= 0 || n_source_points <= 0 || ndata_len <= 0 || data_len <= 0) {
    fprintf(stderr, "ERROR: Failed to load one or more essential .bin data "
                    "files. Check paths and file contents.\n");
    free(spots_mat);
    free(extra_mat);
    free(ndata_store);
    free(data_store);
  ...
  if (!results) {
    fprintf(stderr, "ERROR: Failed to allocate memory for results array.\n");
    return 1;
  }
...
#pragma omp atomic read
      ...
      if ((long)(current_progress / 10000) !=
          (long)((current_progress - 1) / 10000)) {
        printf("Processing... %ld / %ld (%.1f%%)\r", current_progress,
               n_source_points, 100.0 * current_progress / n_source_points);
        fflush(stdout);
      }
    }

    results[i] = -1;
    Point3D query_point = {.y = extra_mat[i * N_COL_EXTRAINFO + 0],
                           .z = extra_mat[i * N_COL_EXTRAINFO + 1],
                           .omega = extra_mat[i * N_COL_EXTRAINFO + 2],
                           .ring = (int)extra_mat[i * N_COL_EXTRAINFO + 5],
                           .eta = extra_mat[i * N_COL_EXTRAINFO + 6]};
    query_point.omega += offset_omega;
    if (query_point.omega < -180)
      query_point.omega += 360.0;
    if (query_point.omega > 180)
      query_point.omega -= 360.0;
...
#pragma omp atomic update
    ...
    convert_to_g_vector(query_point.y, query_point.z, query_point.omega,
                        params.distance, &g_source_x, &g_source_y, &g_source_z);
    double len_sq_source = g_source_x * g_source_x + g_source_y * g_source_y +
                           g_source_z * g_source_z;
    ...
    for (int dr = 0; dr < 1; ++dr) { // only search this ring!!!
      for (int de = -1; de <= 1; ++de) {
        for (int dw = -1; dw <= 1; ++dw) {
          ...
          if (current_ring < 0 || current_ring >= n_ring_bins)
            continue;
          int current_eta = (iEta_center + de) % n_eta_bins;
          if (current_eta < 0)
            current_eta += n_eta_bins;
          int current_ome = (iOme_center + dw) % n_ome_bins;
          if (current_ome < 0)
            current_ome += n_ome_bins;

          long long pos = (long long)current_ring * n_eta_bins * n_ome_bins +
                          (long long)current_eta * n_ome_bins + current_ome;
          int num_points_in_bin = ndata_store[pos * 2];
          int start_offset = ndata_store[pos * 2 + 1];
          // printf("Debug: Query point %ld, Ring %d, Eta bin %d, Ome bin %d ->
          // %d candidates\n", i, current_ring, current_eta, current_ome,
          // num_points_in_bin);
          ...
          for (int k = 0; k < num_points_in_bin; ++k) {
            ...
            convert_to_g_vector(y_cand, z_cand, omega_cand, params.distance,
                                &g_cand_x, &g_cand_y, &g_cand_z);
            double dot_product = g_source_x * g_cand_x + g_source_y * g_cand_y +
                                 g_source_z * g_cand_z;
            double len_sq_cand =
                g_cand_x * g_cand_x + g_cand_y * g_cand_y + g_cand_z * g_cand_z;
            if (len_sq_cand > 0) {
              double cos_angle =
                  dot_product / sqrt(len_sq_source * len_sq_cand);
...
int read_parameters(const char *filepath, AppParams *params) {
  ...
  params->num_ring_numbers = 0;
  params->distance = 0.0;
  while (fgets(line, sizeof(line), fp)) {
    if (sscanf(line, "%s", key) != 1)
      continue;
    if (strcmp(key, "EtaBinSize") == 0)
      sscanf(line, "%s %lf", dummy, &params->eta_bin_size);
    else if (strcmp(key, "OmeBinSize") == 0)
      sscanf(line, "%s %lf", dummy, &params->ome_bin_size);
    else if (strcmp(key, "Distance") == 0 || strcmp(key, "Lsd") == 0)
      ...
      if (params->num_ring_numbers < MAX_N_RINGS) {
        sscanf(line, "%s %d", dummy,
               &params->ring_numbers[params->num_ring_numbers++]);
      }
    }
  }
  fclose(fp);
...
long read_binary_file(const char *filepath, void **buffer,
                      size_t element_size) {
