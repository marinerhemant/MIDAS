...
/*
~/opt/midascuda/cuda/bin/nvcc src/IntegratorFitPeaksGPUStreamCSR.cu -o
bin/IntegratorFitPeaksGPUStreamCSR \
...
#define AREA_THRESHOLD                                                         \
  1e-9 // Minimum accumulated area considered valid in profiling/normalization
...
int queue_push(ProcessQueue *queue, uint16_t dataset_num, int64_t *data,
               size_t num_values) {
...
void *handle_client(void *arg) {
  ...
  while (keep_running) {
    ...
    while (total_bytes_read < TOTAL_MSG_SIZE && keep_running) {
      bytes_read = recv(client_socket, buffer + total_bytes_read,
                        TOTAL_MSG_SIZE - total_bytes_read, 0);
      if (bytes_read <= 0) {
        goto connection_closed; // Exit inner loop, then outer loop check
                                // handles keep_running
      }
    ...
    if (total_bytes_read != TOTAL_MSG_SIZE) {
      fprintf(stderr,
              "Warn: Incomplete message received on socket %d (%d/%zu bytes). "
              "Discarding.\n",
    ...
    if (queue_push(&process_queue, dataset_num, data, num_pixels) < 0) {
      printf("handle_client: queue_push failed, likely shutdown. Discarding "
             "frame %d.\n",
             dataset_num);
      gpuWarnchk(cudaFreeHost(
          data)); // Free the allocated buffer since queue didn't take it
...
void *accept_connections(void *server_fd_ptr) {
  ...
  while (keep_running) {
    ...
    *client_socket_ptr =
    ...
    if (*client_socket_ptr < 0) {
      if (errno == EINTR) {
        // Interrupted by signal (likely SIGINT/SIGTERM), loop again to check
        // keep_running
        free(client_socket_ptr);
        continue;
      }
    ...
    create_rc = pthread_create(&thread_id, NULL, handle_client,
...
int ReadCSRMaps(int expected_rows, size_t expected_cols) {
  ...
  check(read_cols_ll <= 0, "Invalid number of cols (%lld) in CSR header",
        read_cols_ll);
  check(read_nnz < 0, "Invalid non-zero count (%lld) in CSR header", read_nnz);
  check(read_rows != expected_rows,
        "CSR map row count (%d) mismatch with expected (%d from parameters)",
        read_rows, expected_rows);
  check((size_t)read_cols_ll != expected_cols,
        "CSR map column count (%lld) mismatch with expected (%zu from "
        "parameters)",
        read_cols_ll, expected_cols);
  ...
  if (csr_num_nonzeros == 0) {
    printf("Warning: CSR map contains zero non-zero entries. Integration will "
           "yield zeros.\n");
    h_csr_values = NULL;
    h_csr_col_indices = NULL;
    gpuErrchk(cudaMallocHost((void **)&h_csr_row_ptr,
                             (csr_num_rows + 1) * sizeof(int)));
    check(h_csr_row_ptr == NULL,
          "CSR Read: Failed cudaMallocHost for zero-NNZ row_ptr"); // Check
                                                                   // allocation
                                                                   // even for
                                                                   // nnz=0
  ...
  gpuErrchk(cudaMallocHost((void **)&h_csr_values,
                           csr_num_nonzeros * sizeof(double)));
  check(h_csr_values == NULL,
        "CSR Read: Failed cudaMallocHost for h_csr_values"); // CHECK 1
  printf("DEBUG ReadCSRMaps: h_csr_values allocated at %p\n",
         (void *)h_csr_values);

  gpuErrchk(cudaMallocHost((void **)&h_csr_col_indices,
                           csr_num_nonzeros * sizeof(int)));
  check(h_csr_col_indices == NULL,
        "CSR Read: Failed cudaMallocHost for h_csr_col_indices"); // CHECK 2
  ...
  gpuErrchk(cudaMallocHost((void **)&h_csr_row_ptr,
                           (csr_num_rows + 1) * sizeof(int)));
  check(h_csr_row_ptr == NULL,
        "CSR Read: Failed cudaMallocHost for h_csr_row_ptr"); // CHECK 3
  ...
  check(read_val != (size_t)csr_num_nonzeros,
        "Failed read CSR values file (read %zu/%lld elements)", read_val,
        csr_num_nonzeros);
  fclose(f_val);
  ...
  check(read_col != (size_t)csr_num_nonzeros,
        "Failed read CSR col_indices file (read %zu/%lld elements)", read_col,
        csr_num_nonzeros);
  fclose(f_col);
  ...
  check(read_row != (size_t)(csr_num_rows + 1),
        "Failed read CSR row_ptr file (read %zu/%d elements)", read_row,
        csr_num_rows + 1);
  fclose(f_row);
...
// Kernel to calculate initial R, TTh, Eta and *masked* Area for each bin using
// CSR map
__global__ void initialize_PerFrameArr_Area_CSR_kernel(
    double *dPerFrameArr,
    size_t bigArrSize, // Output: R, TTh, Eta, Area arrays (size 4*bigArrSize)
    int nRBins, int nEtaBins, const double *dRBinsLow, const double *dRBinsHigh,
    const double *dEtaBinsLow, const double *dEtaBinsHigh,
    // CSR Map Input
    const double *d_csr_values, const int *d_csr_col_indices,
    const int *d_csr_row_ptr,
    // Detector/Mask Info
    int NrPixelsY, int NrPixelsZ, // Detector dimensions
    const int *dMapMask,
    size_t mapMaskWordCount, // Pixel mask (optional, dMapMask can be NULL)
    double px, double Lsd)   // Geometry params
{
  const size_t idx =
      (size_t)blockIdx.x * blockDim.x + threadIdx.x; // Integration bin index
  if (idx >= bigArrSize) {
    return;
  }
  ...
  for (int l = start_offset; l < end_offset; ++l) {
    ...
    if (dMapMask != NULL && mapMaskWordCount > 0) {
      ...
      if (pixel_idx >= 0 &&
          (size_t)pixel_idx < totalPixels) { // Safe cast for comparison
  ...
  dPerFrameArr[1 * bigArrSize + idx] =
      TwoTheta; // TwoTheta values start at bigArrSize
  dPerFrameArr[2 * bigArrSize + idx] =
      EtaMean; // Eta values start at 2*bigArrSize
  dPerFrameArr[3 * bigArrSize + idx] =
      totArea; // Area values start at 3*bigArrSize
}
...
__global__ void integrate_CSR_noPixelMask(
    ...
    const double *dPerFrameArr) // Input: Pre-calculated Area array (at offset
                                // 3*bigArrSize)
{
  const size_t idx =
      (size_t)blockIdx.x * blockDim.x + threadIdx.x; // Integration bin index
  ...
  if (totArea > AREA_THRESHOLD) { // Only integrate if the bin has significant
                                  // *unmasked* area
    int start_offset = d_csr_row_ptr[idx];
    int end_offset = d_csr_row_ptr[idx + 1];
...
__global__ void sequential_transform_kernel(const int64_t *r, int64_t *w,
                                            int cY, int cZ, int nY, int nZ,
                                            int opt) {
  ...
  case 3:
    ys = zo;
    zs = yo;
    break; // Transpose (caller ensures square)
  default:
...
__global__ void final_transform_process_kernel(const int64_t *r, double *o,
                                               const double *d, int cY, int cZ,
                                               int nY, int nZ, int opt,
                                               bool sub) {
  const size_t N = (size_t)nY * nZ;
  const size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= N) {
...
__global__ void process_direct_kernel(const int64_t *r, double *o,
                                      const double *d, size_t N, bool sub) {
  const size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
  if (i < N) {
...
void ProcessImageGPU(const int64_t *hRaw, double *dProc, const double *dAvgDark,
  ...
  if ((Nopt - 1) % 2 == 0) { // Last intermediate write was to d_b2 (Nopt=1,
                             // 3, 5...) -> read from d_b1
    rP = d_b1;
  } else { // Last intermediate write was to d_b1 (Nopt=2, 4...) -> read from
           // d_b2
    rP = d_b2;
  }
  ...
  final_transform_process_kernel<<<nB, th>>>(rP, dProc, dAvgDark, cY, cZ, nY,
                                             nZ, fOpt, doSub);
  gpuErrchk(cudaPeekAtLastError());
}
...
// Objective function for NLopt: Calculates sum of squared errors for multiple
// Pseudo-Voigt peaks + global background
static double problem_function_global_bg(unsigned n, const double *x,
  ...
  if (nP_peaks <= 0 || (4 * nP_peaks + 1 != n)) {
    fprintf(stderr,
            "Obj Func Err: Invalid number of parameters %u for %d peaks\n", n,
            nP_peaks);
  ...
  if (!calculated_I) {
    fprintf(stderr,
            "Obj Func Err: Failed to allocate memory for calculated profile\n");
  ...
  for (int pN = 0; pN < nP_peaks; ++pN) {
    ...
    for (int i = 0; i < Np_bins; ++i) {
      ...
      if (Sigma > 1e-9) {
        gaussian =
            exp(-diff_sq / (2.0 * sigma_sq)) / (Sigma * sqrt(2.0 * M_PI));
      }
      ...
      calculated_I[i] +=
          Amplitude * (Mix * gaussian + (1.0 - Mix) * lorentzian);
    }
  }
  ...
  for (int i = 1; i < N - 1; ++i) {
    if (data[i] > data[i - 1] && data[i] > data[i + 1] &&
      ...
      if (peakCount < maxPossiblePeaks) {
        ...
        fprintf(stderr,
                "Peak find warn: Exceeded max possible peaks buffer.\n");
        break;
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  check(ReadCSRMaps(bigArrSize, totalPixels) != 1,
        "Failed read/validate CSR map files");
  ...
  gpuErrchk(cudaMalloc(&d_csr_row_ptr,
                       (csr_num_rows + 1) * sizeof(int))); // Always needed
  ...
  if (csr_num_nonzeros > 0) {
    printf("DEBUG Main (Before value/col copy): h_csr_values=%p, "
           "h_csr_col_indices=%p\n",
           (void *)h_csr_values, (void *)h_csr_col_indices);
    check(h_csr_values == NULL,
          "Host CSR values pointer is NULL before GPU copy");
    check(h_csr_col_indices == NULL,
          "Host CSR col indices pointer is NULL before GPU copy");
    printf("Copying CSR values/cols from host (pinned) to GPU...\n");
    gpuErrchk(cudaMemcpy(d_csr_values, h_csr_values,
                         csr_num_nonzeros * sizeof(double),
                         cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_csr_col_indices, h_csr_col_indices,
                         csr_num_nonzeros * sizeof(int),
                         cudaMemcpyHostToDevice));
  } else {
  ...
  check(h_csr_row_ptr == NULL, "Host CSR row pointer (h_csr_row_ptr) is NULL "
                               "before GPU copy"); // <<< THIS IS THE FAILING
                                                   // CHECK
  gpuErrchk(cudaMemcpy(d_csr_row_ptr, h_csr_row_ptr,
                       (csr_num_rows + 1) * sizeof(int),
                       cudaMemcpyHostToDevice));
  ...
  if (darkSubEnabled) {
    ...
    if (nDarkFramesRead > 0) {
      ...
      printf("Dark file specified but no frames read/averaged. Using zeros on "
             "GPU.\n");
    }
  } else {
    gpuErrchk(cudaMalloc(&dAvgDark, totalPixels * sizeof(double)));
  ...
  printf("Initializing static PerFrame array (R, TTh, Eta, Area) using CSR map "
         "on GPU...\n");
  int initTPB = 256;
  ...
  if (wr2D) {
    gpuErrchk(
        cudaMallocHost((void **)&hIntArrFrame, bigArrSize * sizeof(double)));
  ...
  while (keep_running) {
    ...
    integrate_CSR_noPixelMask<<<nrVox, integTPB>>>(
        bigArrSize, Normalize, sumI, d_csr_values, d_csr_col_indices,
        d_csr_row_ptr, dProcessedImage, dIntArrFrame, dSumMatrix,
    ...
    if (firstFrame == 1) {
      ...
      for (int e = 0; e < nEtaBins; ++e) {
        hEta[e] =
            hPerFrame[e + 2 * bigArrSize]; // Eta is at offset 2*bigArrSize
      }
      for (int r = 0; r < nRBins; ++r) {
    ...
    if (pkFit) {
        ...
        if (nSpecP > 0) { // Specific peak locations provided
          ...
          for (int p = 0; p < nSpecP; ++p) {
            ...
            if (bestBin != -1 && minDiff < RBinSize * 2.0) {
              ...
              printf("Warn: Specified peak R=%.4f ignored Frame %d (too far "
                     "from bins).\n",
                     pkLoc[p], currFidx);
            }
          }
          ...
          if (validPeakCount == 0) {
            ...
            Peak *reallocPks =
                (Peak *)realloc(pks, validPeakCount * sizeof(Peak));
            if (reallocPks) {
              pks = reallocPks;
            } // If realloc fails, pks is still valid (but larger)
          }
          ...
          if (doSm) {
            ...
            check(!smoothedData,
                  "pkFit: Malloc failed for smoothedData buffer");
          ...
          currentPeakCount =
              findPeaks(dataToFindPeaks, hR, nRBins, &pks, 0.0, 5);
          if (smoothedData) {
            free(smoothedData);
      ...
      if (currentPeakCount > 0 && pks != NULL) {
        ...
        fitParams = (double *)malloc(nFitParams * sizeof(double));
        check(!fitParams, "pkFit: Malloc fitParams");
        lowerBounds = (double *)malloc(nFitParams * sizeof(double));
        check(!lowerBounds, "pkFit: Malloc lowerBounds");
        upperBounds = (double *)malloc(nFitParams * sizeof(double));
        check(!upperBounds, "pkFit: Malloc upperBounds");

        maxOverallIntensity = 0.0;
        for (int r = 0; r < nRBins; ++r) {
          if (h_int1D[r] > maxOverallIntensity) {
            maxOverallIntensity = h_int1D[r];
          }
        }
        if (maxOverallIntensity <= 0) {
          maxOverallIntensity = 1.0;
        }
        ...
        if (nlopt_rc < 0) {
          ...
          sendFitParams =
              (double *)malloc(currentPeakCount * 5 * sizeof(double));
          check(!sendFitParams,
                "pkFit: Malloc failed for sendFitParams buffer");
        ...
        nlopt_destroy(opt);
        free(fitParams);
        fitParams = NULL; // Free intermediate buffer
        free(lowerBounds);
        lowerBounds = NULL;
        free(upperBounds);
        upperBounds = NULL;
    ...
    if (pkFit && currentPeakCount > 0 && sendFitParams != NULL && fFit) {
      written =
          fwrite(sendFitParams, sizeof(double), currentPeakCount * 5, fFit);
    ...
    printf("F#%d: Ttl:%.2f| QPop:%.2f GPU(Proc:%.2f Int:%.2f Prof:%.2f "
           "D2H:%.2f) CPU(Wr2D:%.2f Wr1D:%.2f Fit:%.2f WrFit:%.2f)\n",
           currFidx, t_loop_cpu, t_qp_cpu, t_proc_gpu, t_integ_gpu, t_prof_gpu,
           t_d2h_gpu, t_write2d_cpu, t_write1d_cpu, t_fit_cpu, t_writefit_cpu);
    fflush(stdout);
  ...
  if (hIntArrFrame) {
    gpuWarnchk(cudaFreeHost(hIntArrFrame));
  }
  if (hPerFrame) {
    gpuWarnchk(cudaFreeHost(hPerFrame));
  }
  if (h_int1D) {
    gpuWarnchk(cudaFreeHost(h_int1D));
  }
  if (hR) {
    free(hR);
  }
  ...
  if (d_csr_values) {
    gpuWarnchk(cudaFree(d_csr_values));
  } // Free CSR GPU buffers
  if (d_csr_col_indices) {
    gpuWarnchk(cudaFree(d_csr_col_indices));
  }
  if (d_csr_row_ptr) {
    gpuWarnchk(cudaFree(d_csr_row_ptr));
  }
  if (dSumMatrix) {
