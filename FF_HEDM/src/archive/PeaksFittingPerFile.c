  ...
  for (i = 0; i < NrPixelsThisRegion; i++) {
    ...
    for (j = 0; j < 8; j++) {
      ...
      for (k = 0; k < NrPixelsThisRegion; k++) {
        if (xNext == PixelPositions[k][0] && yNext == PixelPositions[k][1] &&
            z[k] > (zThis)) {
          isRegionalMax = 0;
        }
      }
...
int Fit2DPeaks(unsigned nPeaks, int NrPixelsThisRegion, double *z,
  ...
  for (i = 0; i < NrPixelsThisRegion; i++) {
    ...
    Etas[i] =
        CalcEtaAngle(-UsefulPixels[i][0] + Ycen, UsefulPixels[i][1] - Zcen);
    if (Rs[i] > RMax)
      RMax = Rs[i];
    if (Rs[i] < RMin)
      RMin = Rs[i];
  ...
  for (i = 0; i < nPeaks; i++) {
    ...
    x[(8 * i) + 2] = CalcNorm2(MaximaPositions[i][0] - Ycen,
                               MaximaPositions[i][1] - Zcen); // Radius
    x[(8 * i) + 3] = CalcEtaAngle(-MaximaPositions[i][0] + Ycen,
                                  MaximaPositions[i][1] - Zcen); // Eta
    x[(8 * i) + 4] = 0.5;                                        // Mu
...
int main(int argc, char *argv[]) {
  ...
  FileNr = atoi(argv[2]);
  RingNr = atoi(argv[3]);
  double Thresh, bc = 1, Ycen, Zcen, IntSat, OmegaStep, OmegaFirstFile, Lsd, px,
                 Width, Wavelength, MaxRingRad;
  ...
  char OutFile[1024];
  sprintf(OutFile, "%s/%s_%0*d_%d_PS.csv", OutFolderName, FileStem, Padding,
          FileNr, RingNr);
  FILE *outfilewrite;
  outfilewrite = fopen(OutFile, "w");
  fprintf(outfilewrite, "SpotID IntegratedIntensity Omega(degrees) YCen(px) "
                        "ZCen(px) IMax Radius(px) Eta(degrees) SigmaR SigmaEta "
                        "NrPixels TotalNrPixelsInPeakRegion nPeaks maxY maxZ "
                        "diffY diffZ rawIMax returnCode\n");
  int KeepSpots = 0;
  ...
  for (RegNr = 1; RegNr <= NrOfReg; RegNr++) {
    ...
    for (i = 0; i < NrPixelsThisRegion; i++) {
      ...
      z[i] =
          ImgCorrBC[((UsefulPixels[i][0]) * NrPixels) + (UsefulPixels[i][1])];
    }
    unsigned nPeaks;
    nPeaks =
        FindRegionalMaxima(z, UsefulPixels, NrPixelsThisRegion, MaximaPositions,
                           MaximaValues, &IsSaturated, IntSat);
    ...
    if (nPeaks > maxNPeaks) {
      // Sort peaks by MaxIntensity, remove the smallest peaks until maxNPeaks,
      // arrays needed MaximaPositions, MaximaValues.
    ...
    int rc =
        Fit2DPeaks(nPeaks, NrPixelsThisRegion, z, UsefulPixels, MaximaValues,
                   MaximaPositions, IntegratedIntensity, IMAX, YCEN, ZCEN, Rads,
                   Etass, Ycen, Zcen, Thresh, NrPx, OtherInfo);
    ...
    for (i = 0; i < nPeaks; i++) {
      ...
      fprintf(
          outfilewrite, "%d %d %d %d %d %f %f %f %d\n", NrPx[i],
          NrPixelsThisRegion, nPeaks, MaximaPositions[i][0],
          MaximaPositions[i][1], (double)MaximaPositions[i][0] - YCEN[i] - Ycen,
          (double)MaximaPositions[i][1] - ZCEN[i] - Zcen, MaximaValues[i], rc);
    }
    SpotIDStart += nPeaks;
