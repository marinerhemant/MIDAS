...
static inline double CalcAngleErrors(int nspots, int nhkls, int nOmegaRanges,
                                     double x[24], double **spotsYZO,
                                     double **hklsIn, double Lsd,
                                     double Wavelength,
                                     double OmegaRange[20][2],
                                     double BoxSize[20][4], double MinEta,
  ...
  double DisplY, DisplZ, ys, zs, Omega, Radius, Theta, lenK, yDet, zDet, omeDet,
      yt, zt, ya, za;
  int detNr;
  ...
  for (nrSp = 0; nrSp < nrMatchedIndexer; nrSp++) {
    ...
    CorrectTiltSpatialDistortion(
        1, DetParams[detNr][9], spotsYZO[nrSp][2], spotsYZO[nrSp][3], pixelsize,
        DetParams[detNr][0], ParamsMatrix[detNr][0], ParamsMatrix[detNr][1],
        ParamsMatrix[detNr][2], DetParams[detNr][4], DetParams[detNr][5],
        DetParams[detNr][6], DetParams[detNr][7], DetParams[detNr][8], &ya,
        &za);
    DisplacementInTheSpot(x[0], x[1], x[2], DetParams[detNr][0], ya, za,
                          spotsYZO[nrSp][4], wedge, chi, &DisplY, &DisplZ);
    yt = ya - DisplY;
    zt = za - DisplZ;
...
static double problem_function(unsigned n, const double *x, double *grad,
  ...
  double Wedge;
  Wedge = f_data->wedge;
  double error = CalcAngleErrors(nSpots, nhkls, nOmeRanges, Inp, SpotInfoAll,
                                 hkls, LsdMean, Wavelength, OmegaRanges,
                                 BoxSizes, MinEta, Wedge, 0.0, f_data->Error);
  if (nIter % 500 == 0) {
...
void FitGrain(double Ini[12], double LsdMean, double OptP[12],
              double NonOptP[5], int NonOptPInt[5], double **SpotInfoAll,
              double OmegaRanges[20][2], double tol[24], double BoxSizes[20][4],
              double **hklsIn, double *Out, double *Error) {
  unsigned n = 24;
...
int main(int argc, char *argv[]) {
  ...
  double tx, ty, tz, Lsd, p0, p1, p2, RhoD, yBC, zBC, wedge, px, a, b, c, alpha,
      beta, gamma, OmegaRanges[20][2], BoxSizes[20][4], MaxRingRad, MaxTtheta,
  ...
  //~ double OptP[12] =
  //{DetParams[0][3],DetParams[1][3],DetParams[2][3],DetParams[3][3], ~
  //DetParams[0][1],DetParams[1][1],DetParams[2][1],DetParams[3][1],DetParams[0][2],
  //~ DetParams[1][2],DetParams[2][2],DetParams[3][2]};
  //~ double tols[24] =
  //{500,500,500,deg2rad*0.0005,deg2rad*0.0005,deg2rad*0.0005,1,1,1,1,1,1, ~
  //5,0.0000001,5,0.0000001,0.0000001,0.0000001,0.0000001,0.0000001,0.0000001,0.0000001,0.0000001,0.0000001};
  //// 250 microns for position, 0.0005 degrees for orient, 1 % for
  //latticeParameter, ~ // 1 degree for tx[4], 1 pixel for yBC, 1 pixel for zBC
  double OptP[12] = {
      DetParams[0][3], DetParams[1][3], DetParams[2][3] - DetParams[0][3],
      DetParams[3][3], DetParams[0][1], DetParams[1][1],
      DetParams[2][1], DetParams[3][1], DetParams[0][2],
      DetParams[1][2], DetParams[2][2], DetParams[3][2]};
  double tols[24] = {500,
                     500,
                     500,
                     ...
                     0.0000001,
                     0.0000001,
                     0.0000001}; // 250 microns for position, 0.0005 degrees for
                                 // orient, 1 % for latticeParameter, 1 degree
                                 // for tx[4], 1 pixel for yBC, 1 pixel for zBC
  ...
  FitGrain(Ini, LsdMean, OptP, NonOptP, NonOptPInt, SpotInfoAll, OmegaRanges,
           tols, BoxSizes, hkls, Out, Error);
  printf("\nInput:\n");
  for (i = 0; i < 12; i++)
    printf("%f ", Ini[i]);
  OptP[2] += OptP[0];
