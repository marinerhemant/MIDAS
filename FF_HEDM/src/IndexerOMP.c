...
int DoIndexing(int SpotIDs,struct TParams Params, int offsetLoc )
{
	...
	printf("ID: %d, Confidence: %lf, nExp: %lf, nObs: %lf, BF: %lf, nPlanes: %d, nOrients: %d\n",SpotIDs,fracMatches,GrainMatches[0][12],GrainMatches[0][13],bestFracTillNow,nPlaneNormals,nOrient);
	FreeMemMatrix( GrainMatches, MAX_N_MATCHES);
	FreeMemMatrix( GrainMatchesT, MAX_N_MATCHES);
	FreeMemMatrix( TheorSpots, nRowsPerGrain);
	FreeMemMatrix( GrainSpots, nRowsPerGrain);
