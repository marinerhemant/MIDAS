...
void CalcFittedMean(int nIndices, int *NrEachIndexBin, int **Indices,
  ...
  for (i = 0; i < nIndices; i++) {
    ...
    for (j = 0; j < NrPtsForFit; j++) {
      ...
      Rmi = Rs[j] - Rstep / 2;
      Rma = Rs[j] + Rstep / 2;
      CalcPeakProfile(Indices, NrEachIndexBin, i, Average, Rmi, Rma, EtaMi,
                      EtaMa, ybc, zbc, px, NrPixels, &RetVal);
      PeakShape[j] = RetVal;
      if (RetVal != 0) {
        AllZero = 0;
      }
    ...
    if (AllZero != 1) {
      ...
      printf("All intensities were 0. i=%d of %d, %f %f %lf %lf\n", i, nIndices,
             IdealRmins[i], IdealRmaxs[i], EtaMi, EtaMa);
      Rfit = 0;
    }
    RMean[i] = Rfit;
    free(NrPts);
...
void FitTiltBCLsd(int nIndices, double *YMean, double *ZMean,
  ...
  // panels on a multi-panel detector in the ooptimization.... Also change
...
int main(int argc, char *argv[]) {
  ...
  for (a = StartNr; a <= EndNr; a++) {
    ...
    FitTiltBCLsd(nIndices, Yc, Zc, IdealTtheta, Lsd, MaxRingRad, ybc, zbc, tx,
                 ...
                 tolBC, tolP, tolP0, tolP1, tolP2, tolP3, px);
