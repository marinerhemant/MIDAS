...
#define nOverlapsMaxPerImage 50000
...
static inline void DepthFirstSearch(int x, int y, int current_label,
  ...
  Positions[current_label * NrPixels * 4 + PositionTrackers[current_label]] =
      (x * NrPixels) + y;
  ...
  for (i = 0; i < NrPixels; ++i) {
    for (j = 0; j < NrPixels; ++j) {
      if ((ConnectedComponents[i * NrPixels + j] == 0) &&
        ...
        if (component >= nOverlapsMaxPerImage - 1)
          break;
...
static double problem_function(unsigned n, const double *x, double *grad,
  ...
  nPeaks = (n - 1) / 8;
  double BG = x[0];
  double IMAX[nPeaks], R[nPeaks], Eta[nPeaks], Mu[nPeaks], SigmaGR[nPeaks],
      SigmaLR[nPeaks], SigmaGEta[nPeaks], SigmaLEta[nPeaks];
  for (i = 0; i < nPeaks; i++) {
    IMAX[i] = x[(8 * i) + 1];
    R[i] = x[(8 * i) + 2];
    Eta[i] = x[(8 * i) + 3];
...
static inline void CalcIntegratedIntensity(int nPeaks, double *x, double *Rs,
                                           double *Etas, int NrPixelsThisRegion,
...
int Fit2DPeaks(unsigned nPeaks, int NrPixelsThisRegion, double *z,
  ...
  for (i = 0; i < NrPixelsThisRegion; i++) {
    Rs[i] = CalcNorm2(UsefulPixels[i * 2 + 0] - Ycen,
                      UsefulPixels[i * 2 + 1] - Zcen);
    Etas[i] = CalcEtaAngle(-UsefulPixels[i * 2 + 0] + Ycen,
                           UsefulPixels[i * 2 + 1] - Zcen);
    if (Rs[i] > RMax)
      RMax = Rs[i];
    if (Rs[i] < RMin)
  ...
  for (i = 0; i < nPeaks; i++) {
    ...
    x[(8 * i) + 2] = CalcNorm2(MaximaPositions[i * 2 + 0] - Ycen,
                               MaximaPositions[i * 2 + 1] - Zcen); // Radius
    x[(8 * i) + 3] = CalcEtaAngle(-MaximaPositions[i * 2 + 0] + Ycen,
                                  MaximaPositions[i * 2 + 1] - Zcen); // Eta
    x[(8 * i) + 4] = 0.5;                                             // Mu
    ...
    // for (j=0;j<9;j++) printf("Args: %lf %lf
  ...
  CalcIntegratedIntensity(nPeaks, x, Rs, Etas, NrPixelsThisRegion,
...
int main(int argc, char *argv[]) {
  ...
  int locImTransOpt, locRingThresh, nRingsThresh = 0, locOmegaRanges,
                                    nOmegaRanges = 0;
  double omegaStart, omegaStep;
  int32_t dsize;
  char *data = NULL;
  double bc = 1, Ycen = 1024, Zcen = 1024, IntSat = 14000, Lsd = 1000000,
         px = 200, Width = 1000;
  double RhoD = 204800, tx = 0, ty = 0, tz = 0, p0 = 0, p1 = 0, p2 = 0, p3 = 0,
         Wavelength = 0.189714;
  ...
  while ((zip_stat_index(arch, count, 0, finfo)) == 0) {
    ...
    if (strstr(finfo->name, "exchange/mask/0.0.0") != NULL) {
      printf(
          "Mask is found, we will separate mask and saturated intensity. "
          "Please ensure saturated intensity is different from mask pixels\n");
      maskLoc = count;
    }
    ...
    if (strstr(finfo->name, "analysis/process/analysis_parameters/MinNrPx/0") !=
        NULL) {
      arr = calloc(finfo->size + 1, sizeof(char));
      fd = zip_fopen_index(arch, count, 0);
      zip_fread(fd, arr, finfo->size);
    ...
    if (strstr(finfo->name, "analysis/process/analysis_parameters/MaxNrPx/0") !=
        NULL) {
      arr = calloc(finfo->size + 1, sizeof(char));
      fd = zip_fopen_index(arch, count, 0);
      zip_fread(fd, arr, finfo->size);
  ...
  int iter;
  for (iter = 0; iter < nImTransOpt; iter++)
    TransOpt[iter] = *(int *)&data[iter * sizeof(int)];
  for (iter = 0; iter < nImTransOpt; iter++)
    printf("Transopt: %d\n", TransOpt[iter]);
  free(s);
  free(data);
  ...
  printf("Data read completely. Total size: %zu bytes, total time taken: %lf "
         "seconds.\n%lf %d %lf %zu\n",
  ...
  if (DoFullImage == 0) {
    ...
    while (fgets(aliner, 1000, hklf) != NULL) {
      sscanf(aliner, "%s %s %s %s %d %s %s %s %s %s %lf", dummy, dummy, dummy,
             dummy, &Rnr, dummy, dummy, dummy, dummy, dummy, &RRd);
  ...
  if (DoFullImage == 1) {
    ...
    for (a = 1; a < NrPixels; a++) {
      for (b = 1; b < NrPixels; b++) {
        ...
        for (thisRingNr = 0; thisRingNr < nRingsThresh; thisRingNr++) {
          if (Rt > RingRads[thisRingNr] - Width &&
              Rt < RingRads[thisRingNr] + Width) {
  ...
  for (thisRingNr = 0; thisRingNr < nRingsThresh; thisRingNr++) {
    printf("RingRads[%d]: %lf Width: %lf\n", thisRingNr, RingRads[thisRingNr],
           Width);
  }
  printf("Width: %lf\n", Width);
  printf("NrPixels: %d\n", NrPixels);
  ...
  double *ImgCorrBCAll, *ImgCorrBCTempAll, *MaxValAll, *zAll, *IntIntAll,
      *ImaxAll, *YcenAll, *ZcenAll, *RAll, *EtaAll, *OIAll;
  int *BoolImageAll, *ConnCompAll, *PosAll, *PosTrackersAll, *MaxPosAll,
      *UsefulPxAll, *NrPxAll;
  char *locDataAll;
  ...
  PosTrackersAll =
      calloc(nOverlapsMaxPerImage * numProcs, sizeof(*PosTrackersAll));
  ...
  int endFileNr =
      (int)(ceil((double)nFrames / (double)nBlocks)) * (blockNr + 1) < nFrames
          ? (int)(ceil((double)nFrames / (double)nBlocks)) * (blockNr + 1)
          : nFrames;
  int nrJobs =
      (int)(ceil((double)(endFileNr - startFileNr) / (double)(numProcs)));
  printf("StartFileNr: %d EndFileNr: %d numProcs: %d nrJobs/proc: %d blockNr: "
         "%d nrBlocks: %d\n",
...
#pragma omp parallel for num_threads(numProcs) private(FileNr) schedule(dynamic)
  for (FileNr = startFileNr; FileNr < endFileNr; FileNr++) {
    ...
    int *BoolImage, *ConnectedComponents, *Positions, *PositionTrackers,
        *MaximaPositions, *UsefulPixels, *NrPx;
    size_t idxoffset;
    idxoffset = NrPixels;
    idxoffset *= NrPixels;
    idxoffset *= procNum;
    size_t asym_idxoffset;
    asym_idxoffset = NrPixelsY;
    asym_idxoffset *= NrPixelsZ;
    asym_idxoffset *= procNum;
...
#pragma omp critical
    ...
    char OutFile[1024];
    sprintf(OutFile, "%s/%s_%06d_PS.csv", OutFolderName, basename(DataFN),
            FileNr + 1);
    FILE *outfilewrite;
    outfilewrite = fopen(OutFile, "w");
    if (outfilewrite == NULL)
      printf("Cannot open %s for writing. Undefined behavior.\n", OutFile);
    fprintf(outfilewrite,
            ...
            "TotalNrPixelsInPeakRegion\tnPeaks\tmaxY\tmaxZ\tdiffY\tdiffZ\trawIM"
            "ax\treturnCode\tretVal\tBG\tSigmaGR\tSigmaLR\tSigmaGEta\t"
            "SigmaLEta\tMU\n");
    char *locData;
    ...
    memset(PositionTrackers, 0,
           nOverlapsMaxPerImage * sizeof(*PositionTrackers));
    NrOfReg = FindConnectedComponents(BoolImage, NrPixels, ConnectedComponents,
    ...
    for (RegNr = 1; RegNr <= NrOfReg; RegNr++) {
      ...
      for (i = 0; i < NrPixelsThisRegion; i++) {
        UsefulPixels[i * 2 + 0] =
            (int)(Positions[RegNr * NrPixels * 4 + i] / NrPixels);
        UsefulPixels[i * 2 + 1] =
            (int)(Positions[RegNr * NrPixels * 4 + i] % NrPixels);
        z[i] = ImgCorrBC[((UsefulPixels[i * 2 + 0]) * NrPixels) +
                         (UsefulPixels[i * 2 + 1])];
      }
      Thresh = GoodCoords[((UsefulPixels[0 * 2 + 0]) * NrPixels) +
                          (UsefulPixels[0 * 2 + 1])];
      unsigned nPeaks;
      nPeaks = FindRegionalMaxima(z, UsefulPixels, NrPixelsThisRegion,
                                  MaximaPositions, MaximaValues, IntSat,
                                  NrPixels, mask);
      if (nPeaks == 0) { // Saturated peaks removed
        TotNrRegions--;
        continue;
      }
      // printf("After RegionNr: %d, NPeaks: %d, NPx:
      // %d\n",RegNr,nPeaks,NrPixelsThisRegion);
      if (nPeaks > maxNPeaks) {
        // Sort peaks by MaxIntensity, remove the smallest peaks until
        // maxNPeaks, arrays needed MaximaPositions, MaximaValues.
      ...
      double retVal = 0;
      int rc = 0;
      // If we don't want to fit, we can just compute weighted center of mass,
      // but first put nPeaks =1; We need {IntegratedIntensity}, {IMAX}, {YCEN},
      // {ZCEN}, {Rads}, {Etass}, sigmaR (0), sigmaEta (0), {NrPx}, returnCode
      // (4), retVal (0)
      // \tBG\tSigmaGR\tSigmaLR\tSigmaGEta\tSigmaLEta\tMU\n" All of these will
      // be 0 OtherInfo is set already, will be 0 and will populate the other
      // values in the line above.
      if (doPeakFit == 0) {
        ...
        for (i = 0; i < NrPixelsThisRegion; i++) {
          ...
          rMEAN[0] += CalcNorm2(-UsefulPixels[i * 2 + 0] + Ycen,
                                UsefulPixels[i * 2 + 1] - Zcen) *
                      z[i];
          etaMEAN[0] += CalcEtaAngle(-UsefulPixels[i * 2 + 0] + Ycen,
                                     UsefulPixels[i * 2 + 1] - Zcen) *
                        z[i];
        }
        ...
        rc = Fit2DPeaks(nPeaks, NrPixelsThisRegion, z, UsefulPixels,
                        MaximaValues, MaximaPositions, IntegratedIntensity,
                        IMAX, YCEN, ZCEN, Rads, Etass, Ycen, Zcen, Thresh, NrPx,
                        OtherInfo, NrPixels, &retVal);
      }
      for (i = 0; i < nPeaks; i++) {
        fprintf(outfilewrite, "%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t",
                (SpotIDStart + i), IntegratedIntensity[i], Omega,
                -YCEN[i] + Ycen, ZCEN[i] + Zcen, IMAX[i], Rads[i], Etass[i]);
        for (j = 0; j < 2; j++)
          fprintf(outfilewrite, "%f\t", OtherInfo[8 * i + 6 + j]);
        fprintf(outfilewrite, "%d\t%d\t%d\t%d\t%d\t%f\t%f\t%f\t%d\t%lf",
                NrPx[i], NrPixelsThisRegion, nPeaks, MaximaPositions[i * 2 + 0],
                MaximaPositions[i * 2 + 1],
    ...
    printf("FrameNr: %d, NrOfRegions: %d, Filtered regions: %d, Number of "
           "peaks: %d, time for preprocessing: %lf, Total time: %lf\n",
