				...
				double *RCens, double *EtaCens,double Ycen, double Zcen, double Thresh, int *NrPx,double *OtherInfo)
{
	...
	for (i=0;i<nPeaks;i++){
		...
		if (atand(initSigmaEta) > MaxEtaWidth) initSigmaEta = tand(MaxEtaWidth)-0.001;
...
int main(int argc, char *argv[]){
	...
	fprintf(outfilewrite,"SpotID IntegratedIntensity Omega(degrees) YCen(px) ZCen(px) IMax Radius(px) Eta(degrees) SigmaR SigmaEta NrPixels TotalNrPixelsInPeakRegion maxY maxZ diffY diffZ rawIMax\n");
	int KeepSpots = 0;
