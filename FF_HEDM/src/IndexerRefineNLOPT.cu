...
__device__ int rb_tree_check(rb_tree *t);
...
__device__ rb_node nil = {NULL, NULL, NULL, 0, BLACK};
#define NIL (&nil)
...
__device__ static void destroy(rb_node *n)
{
     if (n != NIL) {
...
__device__ static rb_node *find_le(rb_node *p, rb_key k, rb_tree *t)
{
     rb_compare compare = t->compare;
     while (p != NIL) {
...
__device__ static rb_node *find_gt(rb_node *p, rb_key k, rb_tree *t)
{
     rb_compare compare = t->compare;
     while (p != NIL) {
	  if (compare(p->k, k) > 0) { /* p->k > k */
...
__device__ void shift_keys(rb_node *n, ptrdiff_t kshift) /* assumes n != NIL */
{
     n->k += kshift;
