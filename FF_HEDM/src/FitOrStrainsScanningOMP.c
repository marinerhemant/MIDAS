...
static
double problem_function_strain(
    unsigned n,
	const double *x,
	double *grad,
	void* f_data_trial)
...
StrainTensorKenesei(int nspots,double **SpotsInfo, double Distance, double wavelength,
		int nhkls,double **hkls, double StrainTensorSample[3][3], double **SpotMatrix, double *RetVal)
{
	int i,j;
	...
	for (i=0;i<nspots;i++){
		...
		dsObs = wavelength/(2*sin(atan((CalcNorm2(SpotsInfo[i][1],SpotsInfo[i][2]))/Distance)/2));
		...
		if (ds0 == 0){
			printf("This ringNr was not detected: %d. Something is wrong. Please contact hsharma@anl.gov to investigate.\n",ringNr);
			return 0;
		}
...
int main(int argc, char *argv[])
{
	...
	for (thisRowNr = 0; thisRowNr < nSptIDs; thisRowNr++){
	    ...
	    printf("Fitvals: Pos: %7.2f %7.2f %7.2f, Orient: %7.2f %7.2f %7.2f, LatC: %6.4f %6.4f %6.4f %7.3f %7.3f %7.3f\n%lf %lf %lf %lf %lf %lf\n%lf %lf %lf\n",
					...
					FinalResult[9],FinalResult[10],FinalResult[11],a,b,c,alph,bet,gamm,ErrorInt1[0],ErrorInt1[1],ErrorInt1[2]);
		...
		# pragma omp critical
		{
			...
			fprintf(outF,"SpotID\tO11\tO12\tO13\tO21\tO22\tO23\tO31\tO32\tO33\tSpotID\tx\ty\tz\tSpotID\ta\tb\tc\talpha\tbeta\tgamma\tSpotID\tPosErr\tOmeErr\tInternalAngle\n");
