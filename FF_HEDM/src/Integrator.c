	...
	for (i=0;i<nFrames;i++){
		...
		for (j=0;j<nRBins;j++){
			for (k=0;k<nEtaBins;k++){
				...
				if ((RBinsLow[j]+RBinsHigh[j])/2 == 507.5 && (EtaBinsLow[k]+EtaBinsHigh[k])/2 == -177.5) printf("%d %lf %lf %lf %lf\n",nPixels,EtaBinsLow[k],EtaBinsHigh[k],Intensity,totArea);
				if ((RBinsLow[j]+RBinsHigh[j])/2 == 507.5 && (EtaBinsLow[k]+EtaBinsHigh[k])/2 ==  182.5) printf("%d %lf %lf %lf %lf\n",nPixels,EtaBinsLow[k],EtaBinsHigh[k],Intensity,totArea);
				if ((RBinsLow[j]+RBinsHigh[j])/2 == 507.5 && (EtaBinsLow[k]+EtaBinsHigh[k])/2 ==  177.5) printf("%d %lf %lf %lf %lf\n",nPixels,EtaBinsLow[k],EtaBinsHigh[k],Intensity,totArea);
				if ((RBinsLow[j]+RBinsHigh[j])/2 == 507.5 && (EtaBinsLow[k]+EtaBinsHigh[k])/2 == -182.5) printf("%d %lf %lf %lf %lf\n",nPixels,EtaBinsLow[k],EtaBinsHigh[k],Intensity,totArea);
			}
		}
