...
int main(int argc, char *argv[]) {
  if (argc < 13) {
    printf("Usage: MatchGrains   OutFileName   state1.txt state2.txt"
           "     SGNr      offset[3]"
           "     matchMode    beamThickness1 beamThickness2    "
           "removeDuplicates       sizeFilter     (optional)weights     "
           "(optional)referenceMisorientation\n");
    printf("                                (list of Grains.csv files)"
           "   [int] [microns][3vals]"
           " (next line)      [microns]     [microns]        binary[0 or 1]    "
           "  %%[0 or value]  [degrees and microns]     [degrees]\n");
    printf("stateN.txt:\n"
           "\t- A file containing a list of Grains.csv files or a Grains.csv "
           "file directly.\n"
           "\t- If a list of Grains.csv files is provided, the results are "
           "merged into a single file.\n"
           "\t  The grains are translated in Z according to the beamThickness "
           "for each state.\n"
    ...
    printf("removeDuplicates: \n"
           "\t0: will not remove any matched grains from database while "
           "matching. This is faster\n"
           "\t   and desirable if multiple grains can be matched to the same "
           "grain, eg. if a \n\t   grain breaks up into 2.\n");
    printf("\t1: will remove any matched grains from database while matching. "
           "Only one grain each will be matched. This is slower.\n");
    printf(
        "\t   sizeFilter: 0 will not make filter based on grain size, value "
        "[float] will only match grains within value%% of the grain size.\n");
    printf("referenceMisorientation: If provided, in degrees, it will look for "
           "misorientation angle around this value.\n");
    printf("                         This is used in cases when it is expected "
           "that grains will have a certain misorientation:\n"
           "                            EG: if the sample frame was rotated "
           "between two steps.\n"
           "                                Or if you want to find grains with "
           "special orientation relationships, such as twins.\n"
           "                                   Provide the twin angle and it "
           "will find twins for you.\n");
    return 1;
  }
  clock_t start, end;
  double diftotal;
  ...
  while (fgets(aline, 4096, f1) != NULL) {
    ...
    if (grainsSupplied == 1) { // Was supplied a grains file directly.
      if (ThisID >= MAX_N_GRAINS) {
        printf("Error: exceeded MAX_N_GRAINS (%d). Increase limit.\n",
               MAX_N_GRAINS);
        return 1;
      }
      sscanf(aline,
             ...
             &OrientMatrix[6], &OrientMatrix[7], &OrientMatrix[8],
             &Pos1[ThisID][0], &Pos1[ThisID][1], &Pos1[ThisID][2], dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, dummy, dummy, &GrSize1[ThisID]);
  ...
  while (fgets(aline, 4096, f2) != NULL) {
    ...
    if (grainsSupplied == 1) { // Was supplied a grains file directly.
      ...
      sscanf(aline,
             ...
             &OrientMatrix[3], &OrientMatrix[4], &OrientMatrix[5],
             &OrientMatrix[6], &OrientMatrix[7], &OrientMatrix[8],
             &Pos2[ThisID][0], &Pos2[ThisID][1], &Pos2[ThisID][2], dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, dummy, dummy, &GrSize2[ThisID]);
  ...
  // for (i=0;i<NrSymmetries;i++) printf("%d %lf %lf %lf
  ...
  if (matchMode == 0) {
    for (i = 0; i < totIDs2; i++) {
      ...
      for (j = 0; j < totIDs1; j++) {
        ...
        if (sizeFilter != 0) {
          if (fabs(GrSize2[i] - GrSize1[j]) > GrSize2[i] * 0.01 * sizeFilter) {
    ...
    for (i = 0; i < totIDs2; i++) {
      ...
      for (j = 0; j < totIDs1; j++) {
        ...
        posT1[1] = Pos1[j][1];
        posT1[2] = Pos1[j][2];
        difflen = Len3d(posT2[0] - posT1[0], posT2[1] - posT1[1],
                        posT2[2] - posT1[2]);
        if (sizeFilter != 0) {
          if (fabs(GrSize2[i] - GrSize1[j]) > GrSize2[i] * 0.01 * sizeFilter) {
  ...
  if (removeDuplicates == 1) {
    ...
    for (i = 0; i < totIDs1 * totIDs2; i++) {
      ...
      Matches[counter * 28 + 27] =
          Len3d(Matches[counter * 28 + 24], Matches[counter * 28 + 25],
                Matches[counter * 28 + 26]);
      counter++;
      if (counter == totIDs2)
        break;
    }
  ...
  fprintf(
      outfile,
      "%%"
      "NewIDState2\tFNrState2\tOrigIDState2\tNewIDState1\tFNrState1\tOrigIDStat"
      "e1\t"
      "Quat0State2\tQuat1State2\tQuat2State2\tQuat3State2\tQuat0State1\tQuat1St"
      "ate1\tQuat2State1\tQuat3State1\t"
      "Pos0State2\tPos1State2\tPos2State2\tPos0State1\tPos1State1\tPos2State1\t"
      "GrainSize1\tGrainSize2\tselectionCriteriaVal\tminAngle\tdiffPosX\tdiffPo"
      "sY\tdiffPosZ\tEuclideanDistt\n");
  for (i = 0; i < totIDs2; i++) {
    if ((int)Matches[i * 28 + 2] == 0)
      continue;
    for (j = 0; j < 6; j++)
      fprintf(outfile, "%d\t", (int)Matches[i * 28 + j]);
    for (j = 6; j < 27; j++)
      fprintf(outfile, "%lf\t", Matches[i * 28 + j]);
    fprintf(outfile, "%lf\n", Matches[i * 28 + 27]);
  }
  // Write unmatched grains from State1
  int thisID1, thisID2, found;
  for (i = 0; i < totIDs1; i++) {
    thisID1 = IDs1[i][2];
    found = 0;
    for (j = 0; j < counter; j++) {
      thisID2 = (int)Matches[j * 28 + 5];
      if (thisID1 == thisID2) {
        found = 1;
        break;
      }
  ...
  // Write unmatched grains from State2
  for (i = 0; i < totIDs2; i++) {
    thisID2 = IDs2[i][2];
    found = 0;
    for (j = 0; j < counter; j++) {
      thisID1 = (int)Matches[j * 28 + 2];
      if (thisID1 == thisID2) {
        found = 1;
        break;
      }
