...
int main(int argc,
         char *argv[]) // Arguments: OldFolder, NewFolder, ParametersFile
{
  printf("Version: %s\n", MIDAS_VERSION_STRING);
  if (argc != 3) {
    printf("Usage:\n GrainTracking OldFolder(where Grains file is located) "
           "Parameters.txt\n");
    return 1;
  }
  int i, j, k;
  ...
  while (fgets(aline, MAX_LINE_LENGTH, GrainsFile) != NULL) {
    sscanf(
        ...
        "%lf %lf %lf %lf %lf %lf",
        &GrainIDsOld[grainNr], &GrainInfo[grainNr][0], &GrainInfo[grainNr][1],
        &GrainInfo[grainNr][2], &GrainInfo[grainNr][3], &GrainInfo[grainNr][4],
        &GrainInfo[grainNr][5], &GrainInfo[grainNr][6], &GrainInfo[grainNr][7],
        &GrainInfo[grainNr][8], &GrainInfo[grainNr][9], &GrainInfo[grainNr][10],
        &GrainInfo[grainNr][11], &GrainInfo[grainNr][12],
        &GrainInfo[grainNr][13], &GrainInfo[grainNr][14],
        &GrainInfo[grainNr][15], &GrainInfo[grainNr][16],
        &GrainInfo[grainNr][17]);
    grainNr++;
  }
  if (grainNr != nGrains) {
  ...
  fgets(aline, MAX_LINE_LENGTH, SpotMatrixFile);
  int spotNr = 0;
  double Omegas[MAX_N_SPOTS], Etas[MAX_N_SPOTS], YLab[MAX_N_SPOTS],
      ZLab[MAX_N_SPOTS], Thetas[MAX_N_SPOTS];
  ...
  for (i = 0; i < nGrains; i++) {
    ...
    do { // Check for both EOF and ID matching GrainID
      sscanf(aline, "%d %s %s %$s %s %s %s", &ID, dummy, dummy, dummy, dummy,
             dummy, dummy);
      if (ID != GrainIDsOld[i]) {
        break;
      }
      sscanf(aline, "%s %s %lf %s %s %s %lf %d %lf %lf %lf %s", dummy, dummy,
             &Omegas[spotNr], dummy, dummy, dummy, &Etas[spotNr],
             &RingNrs[spotNr], &YLab[spotNr], &ZLab[spotNr], &Thetas[spotNr],
             dummy);
      spotNr++;
    ...
    for (j = 0; j < spotNr; j++) {
      ...
      SpotToGv(Distance / lenK, YLab[j] / lenK, ZLab[j] / lenK, Omegas[j],
               Thetas[j], &g01, &g02, &g03);
      ...
      minAngle = 100000;
      for (iSpot = 0; iSpot < nspots;
           iSpot++) { // For each potential match, calculate angle between
                      // gvectors
        ...
        SpotToGv(Distance / lenK, y1 / lenK, z1 / lenK, ome1, theta1, &g11,
                 &g12, &g13);
    ...
    fprintf(outfile, "%lf, %lf, %lf, %lf, %lf, %lf\n", GrainInfo[i][12],
            GrainInfo[i][13], GrainInfo[i][14], GrainInfo[i][15],
            GrainInfo[i][16], GrainInfo[i][17]);
    fprintf(outfile,
            "%lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf\n",
            GrainInfo[i][18], GrainInfo[i][0], GrainInfo[i][1], GrainInfo[i][2],
            GrainInfo[i][3], GrainInfo[i][4], GrainInfo[i][5], GrainInfo[i][6],
            GrainInfo[i][7], GrainInfo[i][8], GrainInfo[i][9], GrainInfo[i][10],
            GrainInfo[i][11]);
