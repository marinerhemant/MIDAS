...
__device__ static inline void gpu_Euler2OrientMat(const RealType euler[3], RealType m[3][3]) {
  RealType psi=euler[0]*deg2rad, phi=euler[1]*deg2rad, theta=euler[2]*deg2rad;
  RealType cps=cos(psi),sps=sin(psi),cph=cos(phi),sph=sin(phi),cth=cos(theta),sth=sin(theta);
...
__device__ static inline void gpu_OrientMat2Euler(RealType m[3][3], RealType E[3]) {
...
__device__ static int gpu_CalcOmega(RealType gx, RealType gy, RealType gz, RealType theta,
    RealType omegas[4], RealType etas[4]) {
  int nsol=0;
  RealType len=sqrt(gx*gx+gy*gy+gz*gz), v=sin(theta*deg2rad)*len;
  if (fabs(gy)<1e-4) {
    if (gx!=0) { RealType c=-v/gx; if(fabs(c)<=1){RealType o=acos(c)*rad2deg;omegas[nsol++]=o;omegas[nsol++]=-o;}}
  } else {
    RealType y2=gy*gy,a=1+(gx*gx)/y2,b=(2*v*gx)/y2,c=(v*v)/y2-1,D=b*b-4*a*c;
    if(D>=0){
      RealType sd=sqrt(D);
      RealType c1=(-b+sd)/(2*a);
      if(fabs(c1)<=1){RealType o1a=acos(c1),o1b=-o1a;
        RealType ea=-gx*cos(o1a)+gy*sin(o1a),eb=-gx*cos(o1b)+gy*sin(o1b);
        omegas[nsol++]=(fabs(ea-v)<fabs(eb-v))?o1a*rad2deg:o1b*rad2deg;}
      RealType c2=(-b-sd)/(2*a);
      if(fabs(c2)<=1){RealType o2a=acos(c2),o2b=-o2a;
        RealType ea=-gx*cos(o2a)+gy*sin(o2a),eb=-gx*cos(o2b)+gy*sin(o2b);
...
__device__ static void gpu_DisplacementInTheSpot(RealType a,RealType b,RealType c,
    RealType xi,RealType yi,RealType zi,RealType omega,RealType wedge,RealType chi,
    RealType *Dy,RealType *Dz) {
...
__device__ static void gpu_CorrectForOme(RealType yc,RealType zc,RealType Lsd,
    RealType OmIni,RealType wl,RealType wedge,RealType *ysO,RealType *zsO,RealType *OmeO){
  RealType CosOme=cos(deg2rad*OmIni),SinOme=sin(deg2rad*OmIni);
  RealType eta=gpu_CalcEtaAngle(yc,zc);
  RealType RR=sqrt(yc*yc+zc*zc),tth=rad2deg*atan(RR/Lsd),th=tth/2;
  RealType SinTh=sin(deg2rad*th),CosTh=cos(deg2rad*th),ds=2*SinTh/wl;
  ...
  RealType G1a=k1f*CosOme+k2f*SinOme,G2a=k2f*CosOme-k1f*SinOme,G3a=k3f;
  ...
  RealType Op1=fabs(SO1*SO1+CO1*CO1-1),Op2=fabs(SO1*SO1+CO2*CO2-1);
  RealType Om1,Om2;
...
__device__ static void gpu_SpotToGv(RealType xi,RealType yi,RealType zi,RealType Omega,
    RealType theta,RealType *g1,RealType *g2,RealType *g3){
  RealType cO=cos(Omega*deg2rad),sO=sin(Omega*deg2rad);
  RealType eta=gpu_CalcEtaAngle(yi,zi),tE=tan(-eta*deg2rad),sTh=sin(theta*deg2rad);
...
__device__ static void gpu_CorrectHKLsLatC(const RealType LatC[6],const RealType *hklsRaw,
    int nhkls,RealType Lsd,RealType wl,RealType *hklsOut){
  RealType a=LatC[0],b=LatC[1],c=LatC[2],al=LatC[3],be=LatC[4],ga=LatC[5];
  RealType SA=sin(al*deg2rad),SB=sin(be*deg2rad),SG=sin(ga*deg2rad);
  RealType CA=cos(al*deg2rad),CB=cos(be*deg2rad),CG=cos(ga*deg2rad);
  RealType GP=acos((CA*CB-CG)/(SA*SB))*rad2deg,BP=acos((CG*CA-CB)/(SG*SA))*rad2deg;
  RealType SBP=sin(BP*deg2rad);
  RealType V=a*b*c*SA*SBP*SG;
  RealType AP=b*c*SA/V,BPr=c*a*SB/V,CP=a*b*SG/V;
  RealType B00=AP,B01=BPr*cos(GP*deg2rad),B02=CP*cos(BP*deg2rad);
  RealType B11=BPr*sin(GP*deg2rad),B12=-CP*SBP*CA,B22=CP*SBP*SA;
  for(int i=0;i<nhkls;i++){
    RealType h=hklsRaw[i*7+0],k=hklsRaw[i*7+1],l=hklsRaw[i*7+2];
    RealType g0=B00*h+B01*k+B02*l,g1=B11*k+B12*l,g2=B22*l;
...
__device__ static int gpu_CalcDiffrSpots(RealType orient[3][3],RealType distance,
    const RealType *omeRanges,int nOmeRanges,const RealType *boxSizes,
    const RealType *hkls,int nhkls,RealType excludePole,RealType *spots){
  int spotnr=0;
  for(int ih=0;ih<nhkls;ih++){
    ...
    RealType theta=hkls[ih*7+4],RR=hkls[ih*7+5],RingNr=hkls[ih*7+6];
    RealType Ds=hkls[ih*7+3],NGc=sqrt(Gc[0]*Gc[0]+Gc[1]*Gc[1]+Gc[2]*Gc[2]);
...
__device__ static RealType gpu_FitErrorsPosT(
    const RealType *x, // [12] = pos(3)+euler(3)+latc(6)
...
__device__ static RealType gpu_FitErrorsOrientStrains(
    const RealType *x, // [9] = euler(3)+latc(6)
    const RealType *spotsYZO, int nSpots,
    const RealType *hklsRaw, RealType *scratch, int nMaxTheor,
    const RealType Pos[3]) {
  RealType *hkls = scratch;
  RealType *theorSpots = scratch + d_params.nhkls*7;
  ...
  for (int sp = 0; sp < nSpots; sp++) {
    ...
    for(int k=0;k<nTspots;k++){
      ...
      RealType gt0=theorSpots[k*9+3],gt1=theorSpots[k*9+4],gt2=theorSpots[k*9+5];
...
__device__ static RealType gpu_FitErrorsStrains(
    const RealType *x, // [6] = latc(6)
    const RealType *spotsYZO, int nSpots,
    const RealType *hklsRaw, RealType *scratch, int nMaxTheor,
  ...
  for (int sp = 0; sp < nSpots; sp++) {
    ...
    for(int k=0;k<nTspots;k++){
      if((int)theorSpots[k*9+8]==spnr){
        RealType dy=ys-theorSpots[k*9+0],dz=zs-theorSpots[k*9+1];
...
__device__ static int gpu_ReassignSpotsFromBins(
    ...
    int maxSpots,
    const RealType *AllSpotsPtr, int totalNSpots,
    const RealType *ObsSpotsLab, int nSpotsBin,
...
__global__ void fitGrainsKernel(
    ...
    RealType *d_spotData,             // [nGrains*MaxNHKLS*11] observed spots per grain (modified in-place)
    ...
    RealType d_Rsample, RealType d_Hbeam,
    RealType d_MargPos, RealType d_MargOme, RealType d_MargABC, RealType d_MargABG,
    ...
    int doDynReassign)
{
  ...
  // CalcAngleErrors initial assignment.
  {
    ...
    for(int sp=0; sp<nSpots; sp++){
      ...
      for(int t=0; t<nInitT; t++){
        RealType tyl=initTheorSpots[t*9+0], tzl=initTheorSpots[t*9+1];
  ...
  if (doDynReassign && d_BinData != nullptr) {
    RealType x12_cur[12] = {res12[0],res12[1],res12[2],
  ...
  RealType finalX[12]={res3[0],res3[1],res3[2],res9[0],res9[1],res9[2],
                     res6[0],res6[1],res6[2],res6[3],res6[4],res6[5]};
  RealType finalErr = gpu_FitErrorsPosT(finalX, spots, nSpots, d_hklsRaw,
                                       scratch, nMaxTheor);
...
int main(int argc, char *argv[]) {
  ...
  FitGPUParams params;
  params.Lsd = (RealType)Lsd; params.Wavelength = (RealType)Wavelength;
  params.MinEta = (RealType)MinEta; params.wedge = (RealType)wedge; params.chi = (RealType)chi;
  ...
  for (int i = 0; i < nOmeRanges; i++) {
    ...
    params.BoxSizes[i*4] = (RealType)BoxSizes[i][0]; params.BoxSizes[i*4+1] = (RealType)BoxSizes[i][1];
    params.BoxSizes[i*4+2] = (RealType)BoxSizes[i][2]; params.BoxSizes[i*4+3] = (RealType)BoxSizes[i][3];
  }
  ...
  RealType *d_initData, *d_spotData, *d_hklsRaw, *d_scratch, *d_LatCin, *d_results;
  ...
  CUDA_CHECK(cudaMemcpy(d_hklsRaw, f_hklsFlat, nHkls7*sizeof(RealType), cudaMemcpyHostToDevice));
  free(f_hklsFlat);

  RealType *f_LatCin = d2f(LatCinT, 6);
  CUDA_CHECK(cudaMalloc(&d_LatCin, 6 * sizeof(RealType)));
  CUDA_CHECK(cudaMemcpy(d_LatCin, f_LatCin, 6*sizeof(RealType), cudaMemcpyHostToDevice));
  free(f_LatCin);
  ...
  CUDA_CHECK(cudaMemcpy(d_initData, f_initData, nInit*sizeof(RealType), cudaMemcpyHostToDevice));
  free(f_initData);
  ...
  CUDA_CHECK(cudaMemcpy(d_spotData, f_spotData, nSpotData*sizeof(RealType), cudaMemcpyHostToDevice));
  free(f_spotData);
  ...
  CUDA_CHECK(cudaMalloc(&d_scratch, (size_t)nSptIDs * scratchPerGrain * sizeof(RealType)));
  ...
  CUDA_CHECK(cudaMemcpy(d_AllSpotsGPU, f_AllSpots, nAllSpots*sizeof(RealType), cudaMemcpyHostToDevice));
  free(f_AllSpots);
  ...
  size_t totalGPUMB = ((size_t)nSptIDs*(15+MaxNSpotsBest*11+scratchPerGrain+27)*sizeof(RealType)
                       + nhkls*7*sizeof(RealType) + (size_t)nSpots*16*sizeof(RealType)) / (1024*1024);
  ...
  fitGrainsKernel<<<blocks, threadsPerBlock>>>(
      ...
      d_scratch, d_LatCin, (RealType)Rsample, (RealType)Hbeam,
      (RealType)MargPos, (RealType)0.01, (RealType)MargABC, (RealType)MargABG,
  ...
  RealType *f_results = (RealType*)malloc((size_t)nSptIDs * 27 * sizeof(RealType));
  CUDA_CHECK(cudaMemcpy(f_results, d_results, (size_t)nSptIDs*27*sizeof(RealType), cudaMemcpyDeviceToHost));
  ...
  for (size_t i = 0; i < (size_t)nSptIDs * 27; i++) h_results[i] = (double)f_results[i];
  free(f_results);
