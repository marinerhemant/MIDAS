...
int main(int argc, char *argv[]) {
  ...
  for (int g = 0; g < nSptIDs; g++) {
    ...
    if (nObs > MaxNSpotsBest || nObs <= 0) {
      printf("    SKIP: nObs=%d out of range\n", nObs);
      continue;
    }
