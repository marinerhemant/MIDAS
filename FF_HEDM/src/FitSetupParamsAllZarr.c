...
void FitTiltBCLsd(int nIndices, double *YMean, double *ZMean, double *IdealTtheta, double Lsd, double MaxRad,
				  ...
				  double *ybcFit, double *zbcFit, double p0, double p1, double p2, double p3, double *MeanDiff, double tolTilts, double tolLsd, double tolBC, double px){
	unsigned n=5;
...
void CorrectTiltSpatialDistortion(int nIndices, double MaxRad, double *YMean, double *ZMean,
		...
		double p2, double p3, double *YCorrected, double *ZCorrected)
{
	double txr,tyr,tzr;
	txr = deg2rad*tx;
	tyr = deg2rad*ty;
...
int main(int argc, char *argv[])
{
    ...
    while ((zip_stat_index(arch, count, 0, finfo)) == 0) {
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/Wedge/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/tolLsd/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/tolBC/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/tolTilts/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MaxOmeSpotIDsToIndex/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MinOmeSpotIDsToIndex/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MinEta/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/DoFit/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(int);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/UseFriedelPairs/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(int);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/OmeBinSize/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MargABG/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MargABC/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MarginOme/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MarginEta/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MarginRadial/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MarginRadius/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/StepSizeOrient/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/tGap/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/tInt/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/StepSizePos/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(double);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/MaxNFrames/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = sizeof(int);
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/BoxSizes/0.0")!=NULL){
            locBoxSizes = count;
        }
        if (strstr(finfo->name,"analysis/process/analysis_parameters/BoxSizes/.zarray")!=NULL){
            s = calloc(finfo->size + 1, sizeof(char));
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, s, finfo->size);
            char *ptr = strstr(s,"shape");
	...
	if (DoFit == 1){
		...
		FitTiltBCLsd(nIndices,Ys,Zs,IdealTtheta,Lsd,RhoD,ybc,zbc,tx,tyIn,tzIn,&ty,&tz,&LsdFit,&ybcFit,&zbcFit,p0,p1,p2,p3,&MeanDiff,tolTilts,tolLsd,tolBC,px);
	...
	CorrectTiltSpatialDistortion(nIndices,RhoD,Ys,Zs,px,LsdFit,ybcFit,zbcFit,tx,ty,tz,p0,p1,p2,p3,YCorrected,ZCorrected);
	...
	fprintf(ExtraInfo,"%YLab ZLab Omega GrainRadius SpotID RingNumber Eta Ttheta OmegaIni(NoWedgeCorr) YOrig(NoWedgeCorr) ZOrig(NoWedgeCorr) YOrig(DetCor) ZOrig(DetCor) OmegaOrig(DetCor) IntegratedIntensity(count)\n");
	for (i=0;i<nIndices;i++){
		if (goodRows[i] == 1){
