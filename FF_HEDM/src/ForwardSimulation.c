...
int main(int argc, char *argv[]) {
  ...
  double LatC[6], Wavelength, Wedge = 0, p0, p1, p2, RhoD, GaussWidth,
                              PeakIntensity = 2000;
  ...
  if (isBin) {
    ...
    if (strncmp(aline, "%NumGrains ", strlen("%NumGrains ")) ==
        0) { // This is a Grains.csv file, get OM, Pos, LatC
      dataType = 0;
      printf(
  ...
  int centIdxMask =
      nrPxMask * 4 * ((int)ceil(GaussWidth)) +
      4 * ((int)ceil(
              GaussWidth)); // This should cover 95% of the total distribution,
                            // centered at 2*ceil(GaussWidth)
  ...
  for (i = 0; i < nrPxMask; i++) {
    for (j = 0; j < nrPxMask; j++) {
      GaussMask[i * nrPxMask + j] =
          exp(-0.5 * (((i - 2 * ceil(GaussWidth)) * (i - 4 * ceil(GaussWidth)) /
                       (GaussWidth * GaussWidth)) +
                      ((j - 2 * ceil(GaussWidth)) * (j - 4 * ceil(GaussWidth)) /
                       (GaussWidth * GaussWidth)))); // i is slow, j is fast
    }
  }
  ...
  for (voxNr = 0; voxNr < nrPoints; voxNr++) {
    ...
    for (spotNr = 0; spotNr < nTspots; spotNr++) {
      ...
      for (i = 0; i < 5; i++)
        Info[i] = TheorSpots[spotNr][i]; // Info has: R,eta,ome,theta,ringnr
      if (nfOutput == 1) {
        ...
        DisplacementSpots(xGrain, yGrain, Lsd, yTemp, zTemp, omeThis, &DisplY2,
                          &DisplZ2);
        xyz[0] = 0;
        xyz[1] = DisplY2;
        xyz[2] = DisplZ2;
        ...
        YZSpotsT[0] = -(outxyz[1]) / px + yBC;
        YZSpotsT[1] = (outxyz[2]) / px +
                      zBC; // The y is calculated in negative way to match FF!
        if (YZSpotsT[0] > NrPixels || YZSpotsT[0] < 0 ||
            YZSpotsT[1] > NrPixels || YZSpotsT[1] < 0)
          continue;
        yDet = YZSpotsT[0];
        zDet = YZSpotsT[1];
      } else {
        ...
        if (omeThis >= OmegaEnd * OmegaStep / fabs(OmegaStep))
          continue;
        if (omeThis < OmegaStart * OmegaStep / fabs(OmegaStep))
          continue;
        // Get diplacements due to spot position
        yTemp = -Info[0] * sin(Info[1] * deg2rad);
        zTemp = Info[0] * cos(Info[1] * deg2rad);
        DisplacementInTheSpot(InputInfo[voxNr][9], InputInfo[voxNr][10],
                              InputInfo[voxNr][11], Lsd, yTemp, zTemp, omeThis,
                              &DisplY2, &DisplZ2);
        yThis = yTemp +
                DisplY2; // These are displaced for grain position, not tilted.
        ...
        if (DisplY == -32100) { // Was not set, check neighbor
          if (idx - NrPixels < 0)
            continue;
          if (idx + NrPixels > NrPixels * NrPixels - 1)
            continue;
          if (yDispl[idx - 1] != -32100.0) {
            ...
            //~ printf("No neighbor was set for tilts. Please check. idx =
            //%lld\n",(long long int)idx); ~ return 1;
            continue;
          }
        }
        yTemp = yThis + DisplY;
        ...
        if (yDet < 0 || yDet >= NrPixels)
          continue;
        if (zDet < 0 || zDet >= NrPixels)
          continue;
        Info[3] =
            0.5 * atand(sqrt(yThis * yThis + zThis * zThis) / Lsd); // New Theta
        ...
        if (writeSpots == 1)
          fprintf(spotsfile,
                  ...
                  spotMatr[8], spotMatr[9], spotMatr[10], spotMatr[11]);
      }
      ...
      yBin = (size_t)yDet;
      zBin = (size_t)zDet;
      imageBin = zBin * NrPixels + yBin; // We do a transpose here to generate
                                         // the correctly oriented GE files.
      centIdx = omeBin + imageBin;
      for (idxNrY = -4 * ceil(GaussWidth); idxNrY <= 4 * ceil(GaussWidth);
           idxNrY++) {
        if ((int)yBin + idxNrY < 0 || (int)yBin + idxNrY >= NrPixels)
          continue;
        for (idxNrZ = -4 * ceil(GaussWidth); idxNrZ <= 4 * ceil(GaussWidth);
             idxNrZ++) {
          if ((int)zBin + idxNrZ < 0 || (int)zBin + idxNrZ >= NrPixels)
            continue;
          ...
          if (currentPos < 0 || currentPos >= ImageArrSize)
            continue;
          ImageArr[currentPos] +=
              (double)(GaussMask[idxNrY * nrPxMask + idxNrZ + centIdxMask] *
                       PeakIntensity);
          if (maxInt < ImageArr[currentPos])
            maxInt = ImageArr[currentPos];
        }
      }
    }
  }
  ...
  if (geOutput == 1) {
    for (i = 0; i < ImageArrSize; i++)
      outArr[i] = (uint16_t)(ImageArr[i] * 15000 / maxInt);
