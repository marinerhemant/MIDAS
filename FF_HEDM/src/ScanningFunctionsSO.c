...
static inline double UpdateArraysThis(double omegaStep, double px, int nVoxels, double *voxelList, double voxelLen,
										double *x, double beamFWHM, int nBeamPositions, double *beamPositions, double omeTol,
	...
	void* f_data_trial)
{
	...
	double err;
	err = UpdateArraysThis(omegaStep, px, nVoxels, voxelList, voxelLen, x, beamFWHM, nBeamPositions, beamPositions, omeTol,
...
void populate_arrays (char *paramFN){
	...
	size_t saveParams[8] = {nVoxels,nhkls,nConn,maxNPos,dataArrSize,sizeFLUT,totalNrSpots,sizeSpotInfoMat};
	...
	fwrite(filteredSpotInfo,sizeSpotInfoMat*sizeof(*filteredSpotInfo),1,filtF);
	fclose(filtF);
...
double evaluateF(){
	// What needs to be read here:
		...
		// 4. Arrays that can be read directly: voxelList, beamPositions, hkls, AllIDsInfo, AllSpotsInfo
	...
	check (Fthis == MAP_FAILED,"mmap %s failed: %s", "/dev/shm/Fthis.bin", strerror(errno));
	close(fd);
	// Read Fthis
	long *FLUT;
