...
void FitTiltBCLsd(int nIndices, double *YMean, double *ZMean,
                  ...
                  double p0, double p1, double p2, double *MeanDiff,
...
static inline void SortSpots(int nIndices, int nCols, double **SpotsInfo) {
  ...
  for (i = 0; i < nIndices; i++) {
    ...
    // for (j=0;j<6;j++) printf("%f ",SpotsInfo[i][j]); printf("\n");
  }
...
int main(int argc, char *argv[]) {
  ...
  char *str, dummy[1000];
  char folder[1024], Folder[1024], *spotsfn, outfolder[1024],
      resultfolder[1024], *idfn, FileStem[1024], fs[1024];
  int StartNr, EndNr, LowNr, NrPixels, LayerNr;
  int SpaceGroup;
  double LatticeConstant[6], Wavelength, MaxRingRad, Lsd, MaxTtheta, TthetaTol,
      ybc, zbc, px, tyIn, tzIn;
  double tx, tolTilts = 1, tolLsd = 5000, tolBC = 1, p0, p1, p2, RhoD, wedge,
             MinEta, OmegaRanges[2000][2], BoxSizes[2000][4];
  int RingNumbers[200], cs = 0, nOmeRanges = 0, nBoxSizes = 0, DoFit = 0,
                        CellStruct = 2, RingToIndex;
  double Rsample, Hbeam, MinMatchesToAcceptFrac, MinOmeSpotIDsToIndex,
      MaxOmeSpotIDsToIndex, Width;
  ...
  double StepSizePos = 5, StepSizeOrient = 0.2, MarginRadius = 500,
         ...
         MarginEta = 500, MarginOme = 0.5, OmegaStep;
  ...
  sprintf(FileName, "%s/PeakSearch/%s/Radius_StartNr_%d_EndNr_%d_RingNr_%d.csv",
          Folder, FileStem, StartNr, EndNr, RingNumbers[0]);
  ...
  while (fgets(aline, 1000, hklf) != NULL) {
    sscanf(aline, "%s %s %s %s %d %s %s %s %lf %s %lf", dummy, dummy, dummy,
           dummy, &Rnr, dummy, dummy, dummy, &tht, dummy, &rrdideal);
    if (tht > MaxTtheta / 2)
      break;
    for (i = 0; i < cs; i++) {
  ...
  if (NewType == 0) {
    ...
    while (fgets(line, 5000, fp) != NULL) {
      sscanf(line,
             "%lf %s %lf %lf %lf %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s "
             "%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %lf "
             "%s %s %s %lf",
             &SpotsInfo[counter][0], dummy, &SpotsInfo[counter][1],
             &SpotsInfo[counter][2], &SpotsInfo[counter][3], dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, dummy, &SpotsInfo[counter][4],
             dummy, dummy, dummy, &SpotsInfo[counter][5]);
      counter++;
    }
  } else if (NewType == 1) {
    fp = fopen(FileName, "r");
    ...
    while (fgets(line, 5000, fp) != NULL) {
      sscanf(line,
             "%lf %s %lf %lf %lf %s %s %s %s %s %s %s %lf %lf %s %lf %s %s %s",
             &SpotsInfo[counter][0], dummy, &SpotsInfo[counter][1],
             &SpotsInfo[counter][2], &SpotsInfo[counter][3], dummy, dummy,
             dummy, dummy, dummy, dummy, dummy, &nFramesThis,
             &SpotsInfo[counter][4], dummy, &SpotsInfo[counter][5], dummy,
             dummy, dummy);
    ...
    while (fgets(line, 5000, fltfile) != NULL) {
      sscanf(
          line,
          "%lf %lf %lf %s %s %s %s %s %s %s %s %s %s %lf %s %s %s %s %s %s %s "
          "%s %s %s %s %s %s %s %s %d %s %s %s %s %s %s %s %s %s %s %s %s",
          &z, &y, &ome, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy,
          dummy, dummy, &sumInt, dummy, dummy, dummy, dummy, dummy, dummy,
          dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy,
          &IDtemp, dummy, dummy, dummy, dummy, dummy, dummy, dummy, dummy,
  ...
  for (i = 0; i < nIndices; i++) {
    ...
    SpotsInfo[i][1] =
        ...
        (t_gap / (t_gap + t_int)) * OmegaStep *
            (1.0 - fabs(2 * Zs[i] - (double)NrPixels) / (double)NrPixels);
    for (j = 0; j < n_hkls; j++) {
  ...
  if (DoFit == 1) {
    ...
    FitTiltBCLsd(nIndices, Ys, Zs, IdealTtheta, Lsd, RhoD, ybc, zbc, tx, tyIn,
                 tzIn, &ty, &tz, &LsdFit, &ybcFit, &zbcFit, p0, p1, p2,
  ...
  CorrectTiltSpatialDistortion(nIndices, RhoD, Ys, Zs, IdealTtheta, px, LsdFit,
                               ybcFit, zbcFit, tx, ty, tz, p0, p1, p2,
                               YCorrected, ZCorrected);
  ...
  for (i = 0; i < NumberSpotsToKeep; i++) {
    ...
    for (j = 0; j < nrUniqueRingNumbers; j++) {
      if (RingNumberThis == UniqueRingNumbers[j]) {
        // RingRadiusThis =
        // sqrt((YCorrWedge[RowNumbersToKeep[i]]*YCorrWedge[RowNumbersToKeep[i]])+(ZCorrWedge[RowNumbersToKeep[i]]*ZCorrWedge[RowNumbersToKeep[i]]));
    ...
    fprintf(
        IndexAll, "%12.5f %12.5f %12.5f %12.5f %12.5f %12.5f %12.5f %12.5f\n",
        YCorrWedge[RowNumbersToKeep[i]], ZCorrWedge[RowNumbersToKeep[i]],
        OmegaCorrWedge[RowNumbersToKeep[i]], SpotsInfo[RowNumbersToKeep[i]][5],
        SpotsInfo[RowNumbersToKeep[i]][0], SpotsInfo[RowNumbersToKeep[i]][4],
        EtaCorrWedge[RowNumbersToKeep[i]],
    ...
    fprintf(
        ...
        OmegaCorrWedge[RowNumbersToKeep[i]], SpotsInfo[RowNumbersToKeep[i]][5],
        SpotsInfo[RowNumbersToKeep[i]][0], SpotsInfo[RowNumbersToKeep[i]][4],
        EtaCorrWedge[RowNumbersToKeep[i]], TthetaCorrWedge[RowNumbersToKeep[i]],
        SpotsInfo[RowNumbersToKeep[i]][1], YCorrected[RowNumbersToKeep[i]],
        ZCorrected[RowNumbersToKeep[i]], SpotsInfo[RowNumbersToKeep[i]][2],
        SpotsInfo[RowNumbersToKeep[i]][3], SpotsInfo[RowNumbersToKeep[i]][1]);
  }
  for (i = 0; i < nrUniqueRingNumbers; i++)
    AverageRingRadius[i] /= NrSpotsPerRing[i];
  fclose(IndexAll);
  ...
  for (i = 0; i < nrUniqueRingNumbers; i++) {
    // fprintf(PF,"RingRadii %f;\n",AverageRingRadius[i]); // Might be a problem
    // for deformed materials?
    fprintf(PF, "RingRadii %f;\n", IdealRs[i]);
  }
