...
int main(int argc, char *argv[])
{
    ...
    double diftotal;
    if (argc != 2){
		printf("******************Supply a Zarr file as argument.******************\n"
		"Zarr file must have all the parameters needed: tx, ty, tz, px, BC, Lsd, RhoD,"
    ...
    while ((zip_stat_index(arch, count, 0, finfo)) == 0) {
        ...
        if (strstr(finfo->name,"analysis/process/analysis_parameters/DistortionFN/0")!=NULL){
            arr = calloc(finfo->size + 1, sizeof(char)); 
            fd = zip_fopen_index(arch, count, 0);
            zip_fread(fd, arr, finfo->size);
            dsize = 4096;
